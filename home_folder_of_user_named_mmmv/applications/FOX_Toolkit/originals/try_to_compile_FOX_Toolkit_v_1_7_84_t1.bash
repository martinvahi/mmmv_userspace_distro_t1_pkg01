...
func_mmmv_bash_boilerplate_version(){
    local S_GUID="1a785cb1-01cf-44ed-a742-e0e1c010c7e7"
...
func_mmmv_init_s_timestamp_if_not_inited_t1(){
    if [ "$S_TIMESTAMP" == "" ]; then
        if [ "`which date 2> /dev/null`" != "" ]; then
            ...
            echo ""
            echo "GUID=='6192ba53-e957-49c1-bb22-e0e1c010c7e7'"
            echo ""
        fi
    fi
} # func_mmmv_init_s_timestamp_if_not_inited_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1(){
    ...
    if [ "$S_FP_ORIG" == "" ]; then 
        ...
        echo "GUID=='e072f644-526d-462f-9842-e0e1c010c7e7'"
        echo ""
    else
        if [ ! -e "$S_FP_ORIG" ]; then 
            SB_S_FP_ORIG_VERIFICATION_FAILED="t"
            ...
            echo ""
            echo "GUID=='572d9ec2-1d4e-4210-9432-e0e1c010c7e7'"
            echo ""
        else
            if [ ! -d "$S_FP_ORIG" ]; then 
                SB_S_FP_ORIG_VERIFICATION_FAILED="t"
                ...
                echo "    S_FP_ORIG==$S_FP_ORIG"
                echo ""
                echo "GUID=='c425a64f-6a42-4557-a332-e0e1c010c7e7'"
                echo ""
            fi
        fi
    fi
    ...
    #        echo "GUID=='70fa5bf7-614c-4f39-81e1-e0e1c010c7e7'"
    #        echo ""
    #    fi
} # func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t2(){
    if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo "GUID=='893ed00c-3d12-4b38-b211-e0e1c010c7e7'"
            echo ""
        else
            func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_assert_nonempty_string_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "GUID=='5db56be3-424b-4d0a-a630-e0e1c010c7e7'"
        echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then 
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then 
            ...
            echo "GUID=='4dcb1603-fa45-47f5-b320-e0e1c010c7e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='11ca12d1-f346-4137-8a50-e0e1c010c7e7'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
            ...
            echo ""
            echo "GUID=='b71df6a0-4eb6-45de-944f-e0e1c010c7e7'"
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_IN" == "" ]; then 
            ...
            echo "GUID=='0ab6939a-c868-41be-933f-e0e1c010c7e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
...
func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='4afef1d3-9cf3-4658-952f-e0e1c010c7e7'"
        echo ""
        SB_NO_ERRORS_YET_1="f"
    fi
    #--------------------
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then 
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then 
            echo "is expected to be initialized to \"t\" before calling this function."
            echo "GUID=='5852d023-de9c-4076-8c4f-e0e1c010c7e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='5fa227b1-7629-4949-972e-e0e1c010c7e7'"
        fi
        if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then 
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
            ...
            echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
            echo ""
            echo "GUID=='250a3372-1220-4df2-823e-e0e1c010c7e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$SB_VARIABLE_VALUE" != "t" ]; then 
            if [ "$SB_VARIABLE_VALUE" != "f" ]; then 
                ...
                echo "GUID=='224be49b-ef8f-4bc5-b83e-e0e1c010c7e7'"
                if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                    echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                fi
                echo ""
...
func_mmmv_report_an_error_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "GUID=='59a35222-beca-4b67-b91d-e0e1c010c7e7'"
        echo ""
    else
        echo ""
        echo -e "\e[31mThe code of this script is flawed.\e[39m"
        ...
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='b16ed48f-060f-4338-accd-e0e1c010c7e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because 
...
func_mmmv_report_missing_from_path_and_do_NOT_exit_t1() {
    ...
    if [ "$S_TMP_1" == "" ] ; then
        ...
        echo "GUID=='20a77bc3-42b9-4a2b-a04d-e0e1c010c7e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because 
...
func_mmmv_verify_that_the_file_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='851c34f2-dc0e-43cf-825c-e0e1c010c7e7'"
        echo ""
        #--------
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='e218ab4b-fed2-4426-991c-e0e1c010c7e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID=='1c6951b5-49cc-4f8b-9d5c-e0e1c010c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='832bdbbc-1a26-4b49-b65b-e0e1c010c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='483cdf83-ea2c-44ce-934b-e0e1c010c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            ...
            echo "GUID=='1e573a85-fc14-434a-b13b-e0e1c010c7e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "This function requires 2 parameters: S_FP, S_GUID_CANDIDATE"
        echo "and has an optional 3. parameter: SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE"
        echo "GUID=='3a36fb62-0c5b-47d3-bc1a-e0e1c010c7e7'"
        echo ""
        #--------
        SB_VERIFICATION_FAILED="t"
    fi
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "Valid values are: \"t\", \"f\", \"\" ."
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='11894de4-8227-41f9-a74a-e0e1c010c7e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='1232b753-9fa2-440f-9e39-e0e1c010c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo -e "\e[31mdoes not exist\e[39m."
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='b4e54cc7-9687-4013-9d49-e0e1c010c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ ! -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='4975bda5-4985-4390-9719-e0e1c010c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='75b2338d-2381-45b7-9449-e0e1c010c7e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_add_bin_2_Z_PATH_and_optionally_share_man_2_MANPATH_t1(){
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_GUID_CANDIDATE" == "" ]; then
            ...
            echo "    S_GUID_CANDIDATE==\"\""
            echo ""
            echo "GUID=='1ca6c5aa-2f7b-4757-ba58-e0e1c010c7e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_FP_INSTALLATION_FOLDER" == "" ]; then
            ...
            echo "GUID=='5241a074-696d-42ae-a448-e0e1c010c7e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
        ...
        if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "" ]; then
            ...
            if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "t" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "f" ]; then
                    ...
                    echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                    echo "GUID=='55705a81-5c2a-457f-ba38-e0e1c010c7e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
            "$S_FP_INSTALLATION_FOLDER" "10d65721-8027-4381-a832-e0e1c010c7e7"
        if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            #--------------
            func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                "$S_FP_INSTALLATION_FOLDER/bin" "58ad1365-8225-4116-a432-e0e1c010c7e7"
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            ...
            if [ "$SB_MAN_FOLDER_OR_NONBROKEN_SYMLINK_TO_IT_EXISTS" == "f" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "t" ]; then
                    ...
                    func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                        "$S_FP_INSTALLATION_FOLDER/share/man" "487baa25-4c39-4c5b-b051-e0e1c010c7e7"
                fi
            else
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='1a133933-ecfa-40e6-af48-e0e1c010c7e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_assert_error_code_zero_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='cb965a11-b4f6-4aa6-b417-e0e1c010c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "GUID=='3c285ab2-c997-44cf-9537-e0e1c010c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_error_code_zero_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='17312ff4-d580-4de8-9c47-e0e1c010c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "Something went wrong. Error code: $S_ERR_CODE"
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='05194933-e523-4f69-9017-e0e1c010c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
...
func_mmmv_exc_verify_S_FP_ORIG_t1() {
    if [ "$S_FP_ORIG" == "" ]; then
        ...
        echo ""
        echo "Aborting script."
        echo "GUID=='e0f3c478-1c9f-448f-99b7-e0e1c010c7e7'"
        echo ""
    ...
    if [ ! -e "$S_FP_ORIG" ]; then
        ...
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='03331022-5395-432d-aaa6-e0e1c010c7e7'"
        echo ""
        exit 1 # exit with an error
    fi
    #------------------------
    if [ ! -d "$S_FP_ORIG" ]; then
        ...
        echo "contains the script that prints this error message."
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='57c4afd1-f491-4849-8256-e0e1c010c7e7'"
        echo ""
        exit 1 # exit with an error
    fi
} # func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_exc_verify_S_FP_ORIG_t2(){
    if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo ""
            echo "has a domain of {\"f\", \"t\"}."
            echo "GUID=='72876cf7-839a-43f2-9646-e0e1c010c7e7'"
            echo ""
        else
            func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_cd_S_FP_ORIG_and_exit_t1(){
    ...
    cd "$S_FP_ORIG"
    func_mmmv_assert_error_code_zero_t2 "$?" \
        "53182984-af7f-4f4f-8951-e0e1c010c7e7"
    exit 0
} # func_mmmv_cd_S_FP_ORIG_and_exit_t1
...
func_mmmv_assert_nonempty_string_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='585fdaa3-3a89-402d-af26-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='29e3ed55-5c8a-43eb-a745-e0e1c010c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_IN" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be something other than an empty string."
        echo "GUID=='5bdb7825-ecf1-4882-8825-e0e1c010c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_sbvar_domain_t_f_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='2d5a9903-b525-42cf-8025-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='b5ebdad4-9df4-4394-a455-e0e1c010c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$SB_VARIABLE_VALUE" != "t" ]; then 
        if [ "$SB_VARIABLE_VALUE" != "f" ]; then 
            ...
            echo ""
            echo "but it is expected to be either \"t\" or \"f\"."
            echo "GUID=='c3d82e05-9e0c-4407-99d5-e0e1c010c7e7'"
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_assert_file_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='5e3d9905-8af9-491f-8a34-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            ...
            echo "GUID=='a4f381ec-e102-4245-a054-e0e1c010c7e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='460a5f94-ff2b-40d1-bd24-e0e1c010c7e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='1284d0e1-cca7-44c9-b224-e0e1c010c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='2dadf4d5-39e6-4787-9d44-e0e1c010c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='42a2de86-1492-4d4f-ba54-e0e1c010c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='b224fd41-5185-4489-8d54-e0e1c010c7e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_assert_folder_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='31bf9057-8ec3-47d7-9414-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            echo -e "\e[31mThis code is flawed. \e[39m"
            echo "GUID=='31b6694d-c5af-4a1b-9e44-e0e1c010c7e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='3301a5e5-c18c-4ec3-8a34-e0e1c010c7e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='651379c9-3d8a-4d32-95d3-e0e1c010c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='03842423-aaa7-43fa-a223-e0e1c010c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ ! -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='95059517-95fd-43ad-9423-e0e1c010c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='44708ce3-bda6-4d47-ad33-e0e1c010c7e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_exit_if_not_on_path_t2() { # S_COMMAND_NAME
    ...
    if [ "$S_LOCAL_VARIABLE" == "" ]; then
        ...
        echo "GUID=='4670ac52-c564-4684-8043-e0e1c010c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1;
    fi
...
func_mmmv_exc_exit_with_an_error_t1(){
    ...
    echo "GUID=='306e5e71-dddf-46a5-8413-e0e1c010c7e7'"
    echo ""
    cd "$S_FP_ORIG"
    exit 1 # exit with an error
} # func_mmmv_exc_exit_with_an_error_t1
...
func_mmmv_exc_exit_with_an_error_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "Aborting script."
        echo "GUID=='4ea0aba5-bf63-4d02-a013-e0e1c010c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    else
        ...
        echo "Aborting script."
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='81c7fdf8-dae9-4429-b5f3-e0e1c010c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_determine_Awk_command_t1() {
    ...
    if [ "$SB_THROW" != "f" ]; then
        ...
        echo "The execution of this Bash script is aborted."
        echo "GUID=='146e45e5-88f7-4a1e-9413-e0e1c010c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1;
    fi
...
func_mmmv_exc_is_file_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then 
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='74bab215-e1ef-4f74-8113-e0e1c010c7e7'"
    ...
    if [ -d "$S_FP_CANDIDATE" ]; then 
        ...
        echo "GUID=='6aa4c071-75bb-4612-8d22-e0e1c010c7e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_is_folder_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then 
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='27ba5b1d-2919-4bd7-b322-e0e1c010c7e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
    if [ ! -d "$S_FP_CANDIDATE" ]; then 
        ...
        echo "GUID=='4e712854-8ba6-48f2-8652-e0e1c010c7e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_create_folder_t1(){
    ...
    if [ "$S_FP_FOLDER" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='52d5ea2f-31f5-44c8-8f22-e0e1c010c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
    ...
    if [ -e "$S_FP_FOLDER" ]; then 
        if [ ! -d "$S_FP_FOLDER" ]; then 
            ...
            echo "Aborting script."
            echo "GUID=='56b7e205-9555-45fd-b232-e0e1c010c7e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1 # exit with an error
        fi
        ...
        wait # just in case
        if [ "$S_TMP_0" != "0" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "9e8314c5-4f15-42f1-a341-e0e1c010c7e7" \
                "S_FP_FOLDER==$S_FP_FOLDER"
        fi
        if [ ! -e "$S_FP_FOLDER" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "358affd5-fc65-41a3-ab30-e0e1c010c7e7" \
...
func_mmmv_exc_assure_tmp_folder_existence_t1() {  # S_FP_TMP, S_GUID
    ...
    if [ "$S_GUID" == "" ]; then
        ...
        echo "GUID=='331e2032-1075-4ac2-a342-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP_TMP" ]; then
        if [ -h "$S_FP_TMP" ]; then
            ...
            func_mmmv_assert_error_code_zero_t1 \
                "$?" "46ce8e03-f05b-4010-a320-e0e1c010c7e7"
            func_mmmv_wait_and_sync_t1
            if [ -h "$S_FP_TMP" ]; then
                echo ""
                echo "The path "
                ...
                echo "GUID==\"$S_GUID\""
                echo "GUID=='4187eb15-1130-47e1-aa42-e0e1c010c7e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
        ...
        mkdir -p "$S_FP_TMP"
        func_mmmv_assert_error_code_zero_t1 \
            "$?" "24b418d1-1228-4532-b930-e0e1c010c7e7"
        func_mmmv_wait_and_sync_t1
        func_mmmv_assert_folder_exists_t1 "$S_FP_TMP" \
            '71a45e97-f611-43bc-9272-e0e1c010c7e7'
    else
        if [ ! -d "$S_FP_TMP" ]; then
            echo ""
            if [ -h "$S_FP_TMP" ]; then
            ...
            echo "GUID==\"$S_GUID\""
            echo "GUID=='1033f503-9f58-4c44-ba32-e0e1c010c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
...
func_mmmv_ln_create_hardlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ "$SB_THROW_ON_INVALID_DATA" == "" ]; then
        ...
        if [ "$SB_THROW_ON_INVALID_DATA" != "t" ]; then
            if [ "$SB_THROW_ON_INVALID_DATA" != "f" ]; then
                ...
                echo ""
                echo "Aborting script."
                echo "GUID=='17c1e044-0330-4738-b351-e0e1c010c7e7'"
                echo ""
    ...
    if [ -e "$S_FP_LINK" ]; then 
        if [ -d "$S_FP_LINK" ]; then # folder or a symlink to a folder
            if [ ! -h "$S_FP_LINK" ]; then # not a symlink, therefore a folder
                ...
                echo "    $S_FP_TARGET"
                echo ""
                echo "GUID=='2a815423-a11e-4724-be41-e0e1c010c7e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            echo "GUID=='34eca193-7f07-450d-bc41-e0e1c010c7e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            if [ -h "$S_FP_LINK" ]; then # a broken symlink, therefore NOT missing
                ...
                echo "must not exist before the call to this function."
                echo "GUID=='ac82a6de-7119-4294-8821-e0e1c010c7e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ -e "$S_FP_TARGET" ]; then
            if [ -d "$S_FP_TARGET" ]; then # a folder or a symlink to a folder
                if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore a folder
                    ...
                    echo "    $S_FP_LINK"
                    echo ""
                    echo "GUID=='48b16211-888f-4cb1-a911-e0e1c010c7e7'"
                    echo ""
                    if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                        exit 1
                    fi
            ...
            if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore missing
                ...
                echo "    $S_FP_LINK"
                echo ""
                echo "GUID=='fb70aa04-a7b6-42cc-b151-e0e1c010c7e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
    ...
    #----------------------------------------------------------------------
    local S_TMP_0="not_set_yet GUID=='1af7ea44-8537-4030-a031-e0e1c010c7e7'"
    if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
        ...
        if [ "$S_TMP_0" != "0" ]; then
            ...
            echo "GUID=='943aac11-0972-42e1-9651-e0e1c010c7e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            if [ -h "$S_FP_TARGET" ]; then
                ...
                if [ ! -h "$S_FP_LINK" ]; then
                    ...
                    echo "GUID=='4af0b5b1-293c-4c7e-9b51-e0e1c010c7e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -e "$S_FP_TARGET" ]; then # symlink to a folder or a file
                        if [ ! -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='43615205-361e-4400-b541-e0e1c010c7e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ -d "$S_FP_TARGET" ]; then
                                if [ ! -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='bf73a8f3-3994-4506-9a50-e0e1c010c7e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='2c404a91-127d-45e0-8850-e0e1c010c7e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
                        ...
                        if [ -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='0b1fa0f8-3c19-4940-af70-e0e1c010c7e7'"
                            echo ""
                            S_TMP_0="t"
                        fi
                    fi
                ...
                if [ ! -e "$S_FP_TARGET" ]; then # just an extra test
                    echo ""
                    echo "Problem detection branch marker "
                    echo "GUID=='388be5d1-e806-4791-9750-e0e1c010c7e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -d "$S_FP_TARGET" ]; then # just an extra test
                        echo ""
                        echo "Problem detection branch marker "
                        echo "GUID=='14dc66df-3c6e-46f3-b830-e0e1c010c7e7'"
                        echo ""
                        S_TMP_0="t"
                    else
                        if [ -h "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='219fbd15-172e-4462-ba50-e0e1c010c7e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ ! -e "$S_FP_LINK" ]; then
                                echo ""
                                echo "Problem detection branch marker "
                                echo "GUID=='505337e2-68f2-494b-9b20-e0e1c010c7e7'"
                                echo ""
                                S_TMP_0="t"
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='1aca8fd2-5432-4856-8c10-e0e1c010c7e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
            ...
            if [ "$S_TMP_0" == "t" ]; then
                ...
                echo "GUID=='cfe46884-77bf-4c48-af10-e0e1c010c7e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
                ...
                if [ "$S_TMP_0" != "f" ]; then
                    ...
                    echo "Aborting script."
                    echo "GUID=='a3c9c238-a775-4886-b63f-e0e1c010c7e7'"
                    echo ""
                    exit 1
                fi
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" != "t" ]; then
            ...
            echo -e "\e[31mThe implementation of this function is flawed. \e[39m"
            echo "Aborting script."
            echo "GUID=='312398ec-ff0c-4ca0-9e1f-e0e1c010c7e7'"
            echo ""
            exit 1
        fi
    fi
...
func_mmmv_assert_environment_variable_set_t1() { 
    ...
    if [ "$S_ENVIR_VALUE" == "" ]; then
        ...
        echo "GUID=='71221bf0-8013-454f-be6f-e0e1c010c7e7'"
        echo ""
        exit 1 # exit with an error
    #else 
...
func_mmmv_iterate_over_array_02n_t1() {
    ...
    if [ "$S_ARRAY_VARIABLE_NAME" == "" ]; then
        ...
        echo "GUID=='243abfab-df53-490b-b84f-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_ITERATION_FUNCTION_NAME" == "" ]; then
        ...
        echo "GUID=='49439d51-1fc9-48ff-b63f-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
...
func_mmmv_include_bashfile_if_possible_t1(){ # S_FP_BASHFILE S_GUID_CANDIDATE SB_THROW_ON_ERROR
    ...
    if [ "$SB_THROW_ON_ERROR" == "" ]; then
        ...
        if [ "$SB_THROW_ON_ERROR" != "t" ]; then
            if [ "$SB_THROW_ON_ERROR" != "f" ]; then
                ...
                echo "GUID=='e3c9bccb-adcf-4ebe-925f-e0e1c010c7e7'"
                echo ""
            fi
        fi
    fi
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='442d9e21-24ee-4da9-a15f-e0e1c010c7e7'"
        echo ""
        if [ "$SB_THROW_ON_ERROR" == "t" ]; then
            exit 1
        fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='439c9064-b6a6-46b2-a82e-e0e1c010c7e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='58254f62-fb36-4877-bc4e-e0e1c010c7e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='51cbb825-62ef-464e-873e-e0e1c010c7e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1 
            fi
...
func_mmmv_include_bashfile_if_possible_t2(){ # S_FP_BASHFILE S_GUID_CANDIDATE 
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='27467ab2-35e7-4aa7-915d-e0e1c010c7e7'"
        echo ""
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='50d71bc5-f03e-447d-843d-e0e1c010c7e7'"
            echo ""
        fi
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='04e807d5-2fd8-45fe-9a2d-e0e1c010c7e7'"
            echo ""
        fi
    fi
    #----------------------------------------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='0e958bf9-4b53-4496-b1fd-e0e1c010c7e7'"
            echo ""
        fi
    fi
...
func_mmmv_assert_exists_on_path_t1() {
    ...
    if [ "$S_TMP_1" == "" ]; then
        ...
        echo "GUID=='571db825-5935-4c23-833d-e0e1c010c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_ln_create_or_overwrite_symlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ ! -e "$S_FP_LINK" ]; then
        ...
        echo ""
        echo "PWD==`pwd`"
        echo "GUID=='3f9f5593-e54e-4008-a25d-e0e1c010c7e7'"
        echo ""
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_assert_Linux_or_BSD_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='a51a4428-585b-4671-975d-e0e1c010c7e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" == "" ]; then
        ...
        echo ""
        echo "GUID=='23449252-7cee-4a23-b71d-e0e1c010c7e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
...
func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='4c579881-f839-46d0-941c-e0e1c010c7e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
            ...
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" != "f" ]; then
                ...
                if [ "$S_GUID_CANDIDATE" != "" ]; then
                    ...
                    func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1 \
                        "$SB_DISPLAY_NONERROR_FEEDBACK" \
                        "SB_DISPLAY_NONERROR_FEEDBACK" \
                        "0523b8ba-75bd-4254-bc1f-e0e1c010c7e7"
                fi
                SB_DISPLAY_NONERROR_FEEDBACK="f"
                #----------------------------------------
            fi
...
# RENESSAATOR_BLOCK_END
    ...
    if [ "$SB_DOMAIN_IS_REACHABLE" == "f" ]; then
        ...
        if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            echo "GUID=='19760932-c891-4815-b65c-e0e1c010c7e7'"
            if [ "$S_GUID_CANDIDATE" != "" ]; then
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
        fi
...
func_mmmv_assert_internet_connection_exists_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='184b5681-b15a-4bef-851c-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "630b20eb-0b3e-4d56-a82f-e0e1c010c7e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_STUDY_INTERNET_AVAILABILITY" == "t" ]; then
        ...
        # The 
        func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1 \
            "466394d5-9479-4192-bf1e-e0e1c010c7e7" \
            "$SB_DISPLAY_NONERROR_FEEDBACK"
        ...
        if [ "$SB_VERIFICATION_FAILED" == "t" ]; then
            ...
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
                ...
                func_mmmv_assert_sbvar_domain_t_f_t1 \
                    ...
                    "SB_VERIFICATION_FAILED" \
                    "c12ec8aa-3f62-4c40-8e3e-e0e1c010c7e7"
            fi
        fi
        #----------------------------------------
    ...
    if [ "$SB_INTERNET_AVAILABLE" == "f" ]; then
        ...
        echo "GUID=='475a05f4-7a06-424d-bf3c-e0e1c010c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_GUI_applications_can_be_launched_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='5180752a-e2d7-4a8a-8e1c-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_OK_TO_CACHE" "SB_OK_TO_CACHE" \
            "4faf2bd3-8184-448d-921d-e0e1c010c7e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_OK_TO_CACHE" == "t" ]; then
        if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_RESULT_CACHED" == "t" ]; then
            ...
            if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_GUI_IS_LAUNCHABLE" == "f" ]; then
                ...
                echo "Aborting script."
                echo "GUID=='38c65b15-dd56-4c4a-852c-e0e1c010c7e7'"
                echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                echo ""
                #--------
                cd "$S_FP_ORIG"
    ...
    if [ "$SB_RUN_TEST" == "t" ]; then
        ...
        if [ "$S_TMP_0" != "124" ]; then
            ...
            echo "Aborting script."
            echo "GUID=='49bc5c49-4d4b-4377-a52c-e0e1c010c7e7'"
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_exc_initialize_wine_C_drive_if_needed_t1(){
    ...
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_DISPLAY_NONERROR_FEEDBACK" "SB_DISPLAY_NONERROR_FEEDBACK" \
            "2286ed53-0617-4026-b75c-e0e1c010c7e7" 
    fi
    #----------------------------------------------------------------------
    ...
    if [ ! -d "$S_FP_C" ]; then # for speed optimisation
        if [ ! -e "$S_FP_C" ]; then
            ...
            func_mmmv_assert_GUI_applications_can_be_launched_t1 \
                "c37ef392-b9e6-4491-ac8b-e0e1c010c7e7" \
            ...
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "c14b29e3-f470-4271-ae6a-e0e1c010c7e7"
            func_mmmv_wait_and_sync_t1
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "332c8176-b9b2-4aa6-b93a-e0e1c010c7e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            #--------------------------------------------------------------
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "5e1d47d4-7b95-4dbb-ad4a-e0e1c010c7e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            #--------------------------------------------------------------
        fi
    fi
...
if [ "$S_FP_DIR" == "" ]; then
    ...
    echo "GUID=='285d9463-44c7-4f58-ba3c-e0e1c010c7e7'"
    echo ""
    if [ "$S_FP_ORIG" != "" ]; then
        func_mmmv_exc_verify_S_FP_ORIG_t1
        cd "$S_FP_ORIG"
...
if [ "$S_FP_ORIG" == "" ]; then
    ...
    echo -e "\e[31mExiting with an error. \e[39m"
    echo "GUID=='159f15c4-26c4-452c-a41c-e0e1c010c7e7'"
    echo ""
    exit 1
else
    func_mmmv_exc_verify_S_FP_ORIG_t2
...
func_mmmv_uncompress_tar_dot_foo_and_enter_t1(){
    ...
    if [ "$SB_COMPRESSION_TYPE_RECOGNIZED" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='5c374222-550b-401a-834b-e0e1c010c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_PARENT_FOLDER" \
        "2f2d44c3-c093-4627-8419-e0e1c010c7e7"
    cd $S_FP_PARENT_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "3c0cad15-cc5b-4789-a959-e0e1c010c7e7"
    #----------------------------------------
    ...
    if [ "$SB_COMPRESSIONLESS_TAR" == "f" ]; then
        func_mmmv_assert_file_exists_t1 "$S_FP_COMPRESSED" \
            "56c1b9a3-ef82-4e2d-8019-e0e1c010c7e7"
        $S_CMD_UNCOMPRESS $S_FP_COMPRESSED
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "140174f4-085b-4629-ae49-e0e1c010c7e7"
        func_mmmv_wait_and_sync_t1
    fi
    #----------------------------------------
    func_mmmv_assert_file_exists_t1 "$S_FP_TAR" \
        "be22bf32-8483-41fd-84f8-e0e1c010c7e7"
    $S_CMD_NICE_PREFIX tar -xf $S_FP_TAR
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "c4cc3418-e67c-4547-bec8-e0e1c010c7e7"
    func_mmmv_wait_and_sync_t1
    #----------------------------------------
    func_mmmv_assert_folder_exists_t1 "$S_FP_NEXT_PARENT_FOLDER" \
        "3161035f-f391-452e-afc8-e0e1c010c7e7"
...
func_first_use_message(){
    ...
    echo "GUID=='14eaba91-261e-4ac9-973b-e0e1c010c7e7'"
    echo ""
    #------------------------------
    cd "$S_FP_ORIG"
    exit 1
...
func_display_help_message_and_exit_if_needed(){
    ...
    if [ "$S_ARGV_0" != "" ]; then
        if [ "`echo \"$S_ARGV_0\" | grep -E '^((([-][-]?)?[hH][eE][lL][pP])|([-]?[hH?]))$' `" == "" ]; then
            ...
            echo "GUID=='46b966a3-8adc-4a8b-935b-e0e1c010c7e7'"
...
func_initialize_configuration_by_initializing_global_variables(){
    ...
    S_FP_INSTALLATION_FOLDER="`cd $S_FP_DIR/../ ; pwd`/v_1_7_84"
    S_FN_SRC_EXTENTIONLESS="2023_09_11_FOX_Toolkit_v_1_7_84"
    S_FN_SRC="$S_FN_SRC_EXTENTIONLESS.tar.lz"
    S_FP_SRC="$S_FP_DIR/$S_FN_SRC"
...
func_main(){
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_DIR" \
        "258d4c23-fa1f-4db8-9248-e0e1c010c7e7"
    cd $S_FP_DIR
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "5354ed72-5820-4312-be57-e0e1c010c7e7"
    #------------------------------
    mkdir -p $S_FP_BUILD_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "fe1b144f-20a7-4fd0-bb57-e0e1c010c7e7"
    func_mmmv_wait_and_sync_t1
    func_mmmv_assert_folder_exists_t1 "$S_FP_BUILD_FOLDER" \
        "5e73c7c5-44e2-4197-9047-e0e1c010c7e7"
    cd $S_FP_BUILD_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "22f2289b-628f-4fc8-8817-e0e1c010c7e7"
    #------------------------------
    func_mmmv_assert_file_exists_t1 "$S_FP_SRC" \
        "5a82f612-5ac5-4dc9-8f16-e0e1c010c7e7"
    $S_CMD_PREFIX_NICE cp -f $S_FP_SRC $S_FP_BUILD_FOLDER/
    func_mmmv_wait_and_sync_t1
    #------------------------------
    S_FP_PARENT_FOLDER="$S_FP_BUILD_FOLDER"
    func_mmmv_uncompress_tar_dot_foo_and_enter_t1 \
        "$S_FN_SRC_EXTENTIONLESS" "lz"
    func_mmmv_uncompress_tar_dot_foo_and_enter_t1 \
        "fox-1.7.84" "gz"
    #------------------------------
    cd $S_FP_PARENT_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "1724f008-9562-4444-b0e6-e0e1c010c7e7"
    $S_CMD_PREFIX_NICE ./configure --prefix=$S_FP_INSTALLATION_FOLDER \
        ...
        --with-xft
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "f02d1eb5-db74-4b09-af16-e0e1c010c7e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    if [ "$MMMV_USERSPACE_DISTRO_T1_SI_N_OF_COMPILATION_THREADS_T1" != "" ]; then
        ...
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "825035a3-cb99-4db6-be56-e0e1c010c7e7"
    else
        $S_CMD_PREFIX_NICE $S_CMD_GNU_MAKE 
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "51dfc869-3b5b-465c-b956-e0e1c010c7e7"
    fi
    func_mmmv_wait_and_sync_t1
    #------------------------------
    mkdir -p $S_FP_INSTALLATION_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "35b9cc5f-ce05-466b-bc55-e0e1c010c7e7"
    func_mmmv_assert_folder_exists_t1 "$S_FP_INSTALLATION_FOLDER" \
        "399410e5-498b-47bc-b245-e0e1c010c7e7"
    #------------------------------
    $S_CMD_PREFIX_NICE $S_CMD_GNU_MAKE install
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "441f8399-d2ed-41aa-80b5-e0e1c010c7e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    echo ""
...
exit 0
#--------------------------------------------------------------------------
# S_VERSION_OF_THIS_FILE="47734602-bb1a-4ded-8655-e0e1c010c7e7"
#==========================================================================
