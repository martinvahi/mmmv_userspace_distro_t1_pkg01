...
function loadButtonAction() {
    ...
    const selectedPackages = pkgButtons.filter(btn => btn.checked)
        .map(btn => pkgButtonGrid.querySelector(`label[for='${btn.id}']`).innerText);
 ...
 */
function loadThisPackage(event) {
...
function showManualForSelectedPackage() {
    ...
    if (button)
        showManualForLabel(pkgButtonGrid.querySelector(`label[for='${button.id}']`));
}
/**
 ...
 */
function showManualForThisPackage(event) {
    event.preventDefault();
    showManualForLabel(event.target);
}
...
for (const pkg of redlogSubPackages)
    manualLookupMap.set(pkg, "REDLOG");
 ...
 */
function showManualForLabel(label) {
    let pkg = label.innerText, lookup = manualLookupMap.get(pkg);
    if (lookup)
        pkg = lookup;
    else
        pkg = pkg.toUpperCase();
