...
class ODESolveTemplate extends Template {
    ...
    result() {
        ...
        let y = this.inputs[1].value.trim(), x;
        // Conditions (optional):
        let conds = this.inputs[3].value.trim();
        // ' means d/dx.
        if (ode.includes("'") || conds.includes("'")) {
            function deprime(text) {
                return text.replace(/(\w+)(\'+)/g, (match, p1, p2) => {
                    let deriv = `df(${p1},${x}`;
        ...
        if (conds.length > 0)
            text += ", {" + conds + "}";
        // Options:
        const optEls = this.pattern.querySelectorAll("input[type=checkbox]:checked");
        for (const optEl of optEls) {
            const option = this.pattern.querySelector(`label[for=${optEl.id}]`).innerText;
    ...
    resetButtonAction() {
        ...
        this.pattern.querySelectorAll("input[type=checkbox]:checked")
