...
func_main_and_all_settings(){
    #func_err_msg_0 "1cc7df24-800f-41b9-a853-42b0a08018e7"
...
func_mmmv_exit_if_not_on_path_t2() { # S_COMMAND_NAME
    ...
    if [ "$S_LOCAL_VARIABLE" == "" ]; then
        ...
        echo "GUID=='2b4635d4-d6cb-4a97-a233-42b0a08018e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1;
    fi
...
func_mmmv_assert_file_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='1819bf05-d85f-4d09-bd32-42b0a08018e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            ...
            echo "GUID=='41f61394-3c3d-47b3-a342-42b0a08018e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='271dcc94-34b6-4f34-be52-42b0a08018e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='73cb1c84-b019-4194-a042-42b0a08018e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo "does not exist."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='3574f771-3607-4d94-9152-42b0a08018e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='383e7124-f055-459c-b342-42b0a08018e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='21477e31-581a-4f1f-b322-42b0a08018e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_assert_folder_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='3363fdd5-798c-4c85-b732-42b0a08018e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            echo "This code is flawed."
            echo "GUID=='211e5995-39b7-44f3-b732-42b0a08018e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='20317845-38a9-47f2-8422-42b0a08018e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='83d58f6d-c6bb-4734-8312-42b0a08018e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo "does not exist."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='7cebfba9-c517-4396-bb32-42b0a08018e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ ! -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='fc48f3d3-15e1-4133-8732-42b0a08018e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='4eaea349-c4c2-45f7-9c22-42b0a08018e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_exc_verify_S_FP_ORIG_t1() {
    if [ "$S_FP_ORIG" == "" ]; then
        ...
        echo "GUID=='5761ca64-c320-4857-ab32-42b0a08018e7'"
        echo ""
        exit 1 # exit with an error
    fi
    #------------------------
    ...
    if [ ! -e "$S_FP_ORIG" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='d5d61b16-2a28-41a2-96b2-42b0a08018e7'"
        echo ""
        exit 1 # exit with an error
    fi
    #------------------------
    if [ ! -d "$S_FP_ORIG" ]; then
        ...
        echo "contains the script that prints this error message."
        echo "Aborting script."
        echo "GUID=='356afbf2-5a01-4524-8d22-42b0a08018e7'"
        echo ""
        exit 1 # exit with an error
    fi
} # func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_create_folder_t1(){
    ...
    if [ "$S_FP_FOLDER" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='402c6c64-a92a-4afa-8e12-42b0a08018e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
    ...
    if [ -e "$S_FP_FOLDER" ]; then 
        if [ ! -d "$S_FP_FOLDER" ]; then 
            ...
            echo "Aborting script."
            echo "GUID=='5a4cc171-d7f9-428b-8812-42b0a08018e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1 # exit with an error
        fi
        ...
        wait # just in case
        if [ "$S_TMP_0" != "0" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "1dc10f15-4e25-4629-9652-42b0a08018e7" \
                "S_FP_FOLDER==$S_FP_FOLDER"
        fi
        if [ ! -e "$S_FP_FOLDER" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "13e89271-5161-499d-9512-42b0a08018e7" \
...
func_file_disassembly(){
    ...
    #------------------------------
    func_mmmv_assert_file_exists_t1  \
        "$S_FP_ALL_IN_ONE" "53fbba21-076c-402c-8742-42b0a08018e7"
    #------------------------------
    local S_TMP_0="M"
...
func_file_reassembly(){
    ...
    func_mmmv_assert_file_exists_t1  \
        "$S_TMP_3" "266d2021-53cb-41fd-9242-42b0a08018e7"
    #------------------------------
    ...
    func_mmmv_wait_and_sync_t1 # for USB-sticks, network drives, etc.
    func_mmmv_assert_file_exists_t1  \
        "$S_FP_REASSEMBLY_RESULT" "5c011514-44a2-4c01-8332-42b0a08018e7"
    echo ""
    ...
    echo ""
    echo "GUID=='6f4b7525-8649-4dc2-8d12-42b0a08018e7'"
    echo ""
} # func_file_reassembly
#--------------------------------------------------------------------------
...
exit 0
#==========================================================================
# S_VERSION_OF_THIS_FILE="92fe6df5-46c9-450d-80d2-42b0a08018e7"
#==========================================================================
