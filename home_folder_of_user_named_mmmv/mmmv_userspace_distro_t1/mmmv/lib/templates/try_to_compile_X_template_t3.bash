...
func_mmmv_bash_boilerplate_version(){
    local S_GUID="2cb5a049-4338-4607-93a7-93e0d0d1c7e7"
...
func_mmmv_init_s_timestamp_if_not_inited_t1(){
    if [ "$S_TIMESTAMP" == "" ]; then
        if [ "`which date 2> /dev/null`" != "" ]; then
            ...
            echo ""
            echo "GUID=='8b680824-c3dc-4ba0-a4a7-93e0d0d1c7e7'"
            echo ""
        fi
    fi
} # func_mmmv_init_s_timestamp_if_not_inited_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1(){
    ...
    if [ "$S_FP_ORIG" == "" ]; then 
        ...
        echo "GUID=='23cd0d91-55c5-4ca0-92a7-93e0d0d1c7e7'"
        echo ""
    else
        if [ ! -e "$S_FP_ORIG" ]; then 
            SB_S_FP_ORIG_VERIFICATION_FAILED="t"
            ...
            echo ""
            echo "GUID=='a7b6d13a-8dca-40ea-a277-93e0d0d1c7e7'"
            echo ""
        else
            if [ ! -d "$S_FP_ORIG" ]; then 
                SB_S_FP_ORIG_VERIFICATION_FAILED="t"
                ...
                echo "    S_FP_ORIG==$S_FP_ORIG"
                echo ""
                echo "GUID=='166d361c-7514-4b71-b177-93e0d0d1c7e7'"
                echo ""
            fi
        fi
    fi
    ...
    #        echo "GUID=='01c45559-c560-40fd-b567-93e0d0d1c7e7'"
    #        echo ""
    #    fi
} # func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t2(){
    if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo "GUID=='1b086216-320b-4fcc-9167-93e0d0d1c7e7'"
            echo ""
        else
            func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_assert_nonempty_string_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "GUID=='5b9d9b31-558d-44cc-8467-93e0d0d1c7e7'"
        echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then 
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then 
            ...
            echo "GUID=='170e09d7-f3dc-4162-9157-93e0d0d1c7e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='704c8229-1aa7-4741-b157-93e0d0d1c7e7'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
            ...
            echo ""
            echo "GUID=='42a58c14-c381-49bc-8557-93e0d0d1c7e7'"
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_IN" == "" ]; then 
            ...
            echo "GUID=='f169f85b-e57c-4bc6-9147-93e0d0d1c7e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
...
func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='149c1325-f01d-4390-8447-93e0d0d1c7e7'"
        echo ""
        SB_NO_ERRORS_YET_1="f"
    fi
    #--------------------
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then 
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then 
            echo "is expected to be initialized to \"t\" before calling this function."
            echo "GUID=='61f81e1d-a59d-4e20-8437-93e0d0d1c7e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='38eefb81-659c-489c-9437-93e0d0d1c7e7'"
        fi
        if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then 
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
            ...
            echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
            echo ""
            echo "GUID=='d9a6da4c-792a-40ed-b137-93e0d0d1c7e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$SB_VARIABLE_VALUE" != "t" ]; then 
            if [ "$SB_VARIABLE_VALUE" != "f" ]; then 
                ...
                echo "GUID=='5d2aa964-d552-4535-a827-93e0d0d1c7e7'"
                if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                    echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                fi
                echo ""
...
func_mmmv_report_an_error_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "GUID=='3e5d5ff1-0cac-44bf-9427-93e0d0d1c7e7'"
        echo ""
    else
        echo ""
        echo -e "\e[31mThe code of this script is flawed.\e[39m"
        ...
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='45b25451-6536-45c2-8327-93e0d0d1c7e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because 
...
func_mmmv_report_missing_from_path_and_do_NOT_exit_t1() {
    ...
    if [ "$S_TMP_1" == "" ] ; then
        ...
        echo "GUID=='3c9da733-e88e-43c6-b317-93e0d0d1c7e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because 
...
func_mmmv_verify_that_the_file_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='9b3ab95d-42d5-4ad4-9117-93e0d0d1c7e7'"
        echo ""
        #--------
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='97e1b825-210d-422f-b317-93e0d0d1c7e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID=='18d13631-0175-46f7-b507-93e0d0d1c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='41cf0558-2859-4169-a507-93e0d0d1c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='11ba7a4a-b2de-4cbe-93f6-93e0d0d1c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            ...
            echo "GUID=='4794f574-4c2d-4528-92f6-93e0d0d1c7e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "This function requires 2 parameters: S_FP, S_GUID_CANDIDATE"
        echo "and has an optional 3. parameter: SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE"
        echo "GUID=='e168101a-c190-4c61-aaf6-93e0d0d1c7e7'"
        echo ""
        #--------
        SB_VERIFICATION_FAILED="t"
    fi
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "Valid values are: \"t\", \"f\", \"\" ."
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='56d47b42-790d-43da-9fe6-93e0d0d1c7e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='8d1f6e3a-7c1f-4e8a-93e6-93e0d0d1c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo -e "\e[31mdoes not exist\e[39m."
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='5fab933b-bd2e-4695-91e6-93e0d0d1c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ ! -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='305bb112-1a2f-478d-94d6-93e0d0d1c7e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='067dd219-c01c-41bf-b1d6-93e0d0d1c7e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_add_bin_2_Z_PATH_and_optionally_share_man_2_MANPATH_t1(){
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_GUID_CANDIDATE" == "" ]; then
            ...
            echo "    S_GUID_CANDIDATE==\"\""
            echo ""
            echo "GUID=='fe23dc33-c949-419c-b5c6-93e0d0d1c7e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_FP_INSTALLATION_FOLDER" == "" ]; then
            ...
            echo "GUID=='ee288524-9b17-40ea-b4c6-93e0d0d1c7e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
        ...
        if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "" ]; then
            ...
            if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "t" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "f" ]; then
                    ...
                    echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                    echo "GUID=='35722a38-cab4-4676-a2c6-93e0d0d1c7e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
            "$S_FP_INSTALLATION_FOLDER" "62a2f97b-ec1f-4df6-9577-93e0d0d1c7e7"
        if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            #--------------
            func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                "$S_FP_INSTALLATION_FOLDER/bin" "b6fb9927-9b58-40fb-9477-93e0d0d1c7e7"
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            ...
            if [ "$SB_MAN_FOLDER_OR_NONBROKEN_SYMLINK_TO_IT_EXISTS" == "f" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "t" ]; then
                    ...
                    func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                        "$S_FP_INSTALLATION_FOLDER/share/man" "e4a82a57-530f-4911-b277-93e0d0d1c7e7"
                fi
            else
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='2dfeba54-8ddb-4f79-91b6-93e0d0d1c7e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_assert_error_code_zero_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='515de320-d0de-42f0-85b6-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "GUID=='4c1c04c4-3a51-480a-a2a6-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_error_code_zero_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='82a5dc2d-c091-4699-a5a6-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "Something went wrong. Error code: $S_ERR_CODE"
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='f502b43a-aedf-41bf-95a6-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
...
func_mmmv_exc_verify_S_FP_ORIG_t1() {
    if [ "$S_FP_ORIG" == "" ]; then
        ...
        echo ""
        echo "Aborting script."
        echo "GUID=='1b83832a-10a6-47db-8496-93e0d0d1c7e7'"
        echo ""
    ...
    if [ ! -e "$S_FP_ORIG" ]; then
        ...
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='034eab37-1ab1-4056-8296-93e0d0d1c7e7'"
        echo ""
        exit 1 # exit with an error
    fi
    #------------------------
    if [ ! -d "$S_FP_ORIG" ]; then
        ...
        echo "contains the script that prints this error message."
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='42611d99-651e-445e-8496-93e0d0d1c7e7'"
        echo ""
        exit 1 # exit with an error
    fi
} # func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_exc_verify_S_FP_ORIG_t2(){
    if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo ""
            echo "has a domain of {\"f\", \"t\"}."
            echo "GUID=='f2527c54-0b48-472d-9286-93e0d0d1c7e7'"
            echo ""
        else
            func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_cd_S_FP_ORIG_and_exit_t1(){
    ...
    cd "$S_FP_ORIG"
    func_mmmv_assert_error_code_zero_t2 "$?" \
        "fd75cd24-56eb-4e8e-b567-93e0d0d1c7e7"
    exit 0
} # func_mmmv_cd_S_FP_ORIG_and_exit_t1
...
func_mmmv_assert_nonempty_string_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='2007a791-771b-40ec-b186-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='fd789150-1a9d-4baf-9376-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_IN" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be something other than an empty string."
        echo "GUID=='b599d73b-e7a7-4233-9376-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_sbvar_domain_t_f_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='c954da1b-de83-4c47-9476-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='ed0ba441-c043-47f9-b566-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$SB_VARIABLE_VALUE" != "t" ]; then 
        if [ "$SB_VARIABLE_VALUE" != "f" ]; then 
            ...
            echo ""
            echo "but it is expected to be either \"t\" or \"f\"."
            echo "GUID=='1fd3da32-29a4-4037-b166-93e0d0d1c7e7'"
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_assert_file_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='c33aa733-5c49-4a43-a156-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            ...
            echo "GUID=='772bf825-be44-470b-8256-93e0d0d1c7e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='6cd1ac16-c45b-42a7-9156-93e0d0d1c7e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='db2ee334-f764-4010-a246-93e0d0d1c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='383e8135-85a4-4dad-8146-93e0d0d1c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='b7446f2d-71d3-4c79-b546-93e0d0d1c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='34f85291-b085-43d5-8346-93e0d0d1c7e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_assert_folder_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='b6c56971-7047-4fb9-9336-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            echo -e "\e[31mThis code is flawed. \e[39m"
            echo "GUID=='180df257-be56-402e-a136-93e0d0d1c7e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='ca153e1c-26f2-4303-b536-93e0d0d1c7e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='442c2920-2cba-4549-9536-93e0d0d1c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='44a4f337-7408-4bda-9436-93e0d0d1c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ ! -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='647a942a-af86-4924-b426-93e0d0d1c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='2950381b-da84-44b0-8226-93e0d0d1c7e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_exit_if_not_on_path_t2() { # S_COMMAND_NAME
    ...
    if [ "$S_LOCAL_VARIABLE" == "" ]; then
        ...
        echo "GUID=='01eacb09-fc7b-4be5-b126-93e0d0d1c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1;
    fi
...
func_mmmv_exc_exit_with_an_error_t1(){
    ...
    echo "GUID=='7077dd1a-a149-4761-8126-93e0d0d1c7e7'"
    echo ""
    cd "$S_FP_ORIG"
    exit 1 # exit with an error
} # func_mmmv_exc_exit_with_an_error_t1
...
func_mmmv_exc_exit_with_an_error_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "Aborting script."
        echo "GUID=='5fa90c32-43a4-4124-b126-93e0d0d1c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    else
        ...
        echo "Aborting script."
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='3deb5dc3-abe8-4541-b426-93e0d0d1c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_determine_Awk_command_t1() { # prefers the GNU Awk to the BSD Awk
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "8db34a3d-3c97-42fd-9167-93e0d0d1c7e7" 
    ...
    if [ "$SB_REINIT" == "t" ]; then
        ...
        local SB_THROW="f"
        local S_TMP_0="3ed0b4b2-7ffd-40d5-9167-93e0d0d1c7e7"
        ...
        if [ "$SB_THROW" == "t" ]; then
            ...
            echo "The execution of this Bash script is aborted."
            echo "GUID=='1a787342-554f-4db3-9116-93e0d0d1c7e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1;
        fi
...
func_mmmv_exc_determine_sed_command_t1() { # prefers the GNU sed to the BSD sed
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "1a5ae973-88a1-4823-9257-93e0d0d1c7e7" 
        SB_AVOID_REINIT="$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED"
    fi
    #----------------------------------------------------------------------
    local SB_REINIT="t"
    ...
    if [ "$SB_REINIT" == "t" ]; then
        ...
        local SB_THROW="f"
        local S_TMP_0="3124e073-2fa9-42a3-b457-93e0d0d1c7e7"
        local S_FP_0="/tmp/this/proB9bably/do00es/noTt/eXXXist/ㄫ/$S_TMP_0"
        #------------------------------------------------------------------
        ...
        if [ "$SB_THROW" != "f" ]; then
            ...
            echo "The execution of this Bash script is aborted."
            echo "GUID=='a6ee6225-93c3-4a08-9516-93e0d0d1c7e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1;
        fi
...
func_mmmv_exc_is_file_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then 
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='1c0508e3-5a7d-478c-9116-93e0d0d1c7e7'"
    ...
    if [ -d "$S_FP_CANDIDATE" ]; then 
        ...
        echo "GUID=='3bd3a185-b0ad-4b1a-b316-93e0d0d1c7e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_is_folder_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then 
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='c5d3b75e-605f-458f-9306-93e0d0d1c7e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
    if [ ! -d "$S_FP_CANDIDATE" ]; then 
        ...
        echo "GUID=='1fbbfb03-c8d7-4a7e-a806-93e0d0d1c7e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_create_folder_t1(){
    ...
    if [ "$S_FP_FOLDER" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='222017cb-ad68-4d82-9306-93e0d0d1c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
    ...
    if [ -e "$S_FP_FOLDER" ]; then 
        if [ ! -d "$S_FP_FOLDER" ]; then 
            ...
            echo "Aborting script."
            echo "GUID=='8cecdfc1-c71e-41aa-8406-93e0d0d1c7e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1 # exit with an error
        fi
        ...
        wait # just in case
        if [ "$S_TMP_0" != "0" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "b0d7551e-0eb9-48d2-a547-93e0d0d1c7e7" \
                "S_FP_FOLDER==$S_FP_FOLDER"
        fi
        if [ ! -e "$S_FP_FOLDER" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "c98fd616-0050-4c6f-b547-93e0d0d1c7e7" \
...
func_mmmv_exc_assure_tmp_folder_existence_t1() {  # S_FP_TMP, S_GUID
    ...
    if [ "$S_GUID" == "" ]; then
        ...
        echo "GUID=='8e31403f-b394-41b4-8106-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP_TMP" ]; then
        if [ -h "$S_FP_TMP" ]; then
            ...
            func_mmmv_assert_error_code_zero_t1 \
                "$?" "933a693a-f3d5-4eae-b347-93e0d0d1c7e7"
            func_mmmv_wait_and_sync_t1
            if [ -h "$S_FP_TMP" ]; then
                echo ""
                echo "The path "
                ...
                echo "GUID==\"$S_GUID\""
                echo "GUID=='255f0518-dea7-4d3a-b5f5-93e0d0d1c7e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
        ...
        mkdir -p "$S_FP_TMP"
        func_mmmv_assert_error_code_zero_t1 \
            "$?" "6742f14c-5936-4444-a537-93e0d0d1c7e7"
        func_mmmv_wait_and_sync_t1
        func_mmmv_assert_folder_exists_t1 "$S_FP_TMP" \
            'e6a99215-3ce8-4d76-a1f5-93e0d0d1c7e7'
    else
        if [ ! -d "$S_FP_TMP" ]; then
            echo ""
            if [ -h "$S_FP_TMP" ]; then
            ...
            echo "GUID==\"$S_GUID\""
            echo "GUID=='963e6353-2bef-4464-a5f5-93e0d0d1c7e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
...
func_mmmv_ln_create_hardlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ "$SB_THROW_ON_INVALID_DATA" == "" ]; then
        ...
        if [ "$SB_THROW_ON_INVALID_DATA" != "t" ]; then
            if [ "$SB_THROW_ON_INVALID_DATA" != "f" ]; then
                ...
                echo ""
                echo "Aborting script."
                echo "GUID=='9231c02b-f5f7-4e4c-a1f5-93e0d0d1c7e7'"
                echo ""
    ...
    if [ -e "$S_FP_LINK" ]; then 
        if [ -d "$S_FP_LINK" ]; then # folder or a symlink to a folder
            if [ ! -h "$S_FP_LINK" ]; then # not a symlink, therefore a folder
                ...
                echo "    $S_FP_TARGET"
                echo ""
                echo "GUID=='62ebbe3e-a8c6-46a2-81f5-93e0d0d1c7e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            echo "GUID=='42f5883a-9cfb-433a-94e5-93e0d0d1c7e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            if [ -h "$S_FP_LINK" ]; then # a broken symlink, therefore NOT missing
                ...
                echo "must not exist before the call to this function."
                echo "GUID=='c3b5b84c-e217-42db-81e5-93e0d0d1c7e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ -e "$S_FP_TARGET" ]; then
            if [ -d "$S_FP_TARGET" ]; then # a folder or a symlink to a folder
                if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore a folder
                    ...
                    echo "    $S_FP_LINK"
                    echo ""
                    echo "GUID=='2e3d72ea-d830-45b7-91e5-93e0d0d1c7e7'"
                    echo ""
                    if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                        exit 1
                    fi
            ...
            if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore missing
                ...
                echo "    $S_FP_LINK"
                echo ""
                echo "GUID=='0d46933a-072f-4ae6-83e5-93e0d0d1c7e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
    ...
    #----------------------------------------------------------------------
    local S_TMP_0="not_set_yet GUID=='65afc243-c22f-4bbf-94e5-93e0d0d1c7e7'"
    if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
        ...
        if [ "$S_TMP_0" != "0" ]; then
            ...
            echo "GUID=='ebd14a42-944c-4320-b4d5-93e0d0d1c7e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            if [ -h "$S_FP_TARGET" ]; then
                ...
                if [ ! -h "$S_FP_LINK" ]; then
                    ...
                    echo "GUID=='b67c0554-4a08-4396-81d5-93e0d0d1c7e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -e "$S_FP_TARGET" ]; then # symlink to a folder or a file
                        if [ ! -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='252ab71d-f167-41a3-a3d5-93e0d0d1c7e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ -d "$S_FP_TARGET" ]; then
                                if [ ! -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='4d471c20-2ede-460f-94d5-93e0d0d1c7e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='6b720240-c9fb-45ae-a3d5-93e0d0d1c7e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
                        ...
                        if [ -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='370009f4-c669-43da-84d5-93e0d0d1c7e7'"
                            echo ""
                            S_TMP_0="t"
                        fi
                    fi
                ...
                if [ ! -e "$S_FP_TARGET" ]; then # just an extra test
                    echo ""
                    echo "Problem detection branch marker "
                    echo "GUID=='923e7e46-3e15-44cc-a4c5-93e0d0d1c7e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -d "$S_FP_TARGET" ]; then # just an extra test
                        echo ""
                        echo "Problem detection branch marker "
                        echo "GUID=='72e8d12d-faaf-435d-83c5-93e0d0d1c7e7'"
                        echo ""
                        S_TMP_0="t"
                    else
                        if [ -h "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='c3b4df94-040e-4c24-b5c5-93e0d0d1c7e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ ! -e "$S_FP_LINK" ]; then
                                echo ""
                                echo "Problem detection branch marker "
                                echo "GUID=='87c4ec21-116e-420b-b4c5-93e0d0d1c7e7'"
                                echo ""
                                S_TMP_0="t"
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='cfded557-52c9-4d92-b1b5-93e0d0d1c7e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
            ...
            if [ "$S_TMP_0" == "t" ]; then
                ...
                echo "GUID=='3dbfb811-64f7-42bd-94b5-93e0d0d1c7e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
                ...
                if [ "$S_TMP_0" != "f" ]; then
                    ...
                    echo "Aborting script."
                    echo "GUID=='22ff951d-07a4-4926-83b5-93e0d0d1c7e7'"
                    echo ""
                    exit 1
                fi
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" != "t" ]; then
            ...
            echo -e "\e[31mThe implementation of this function is flawed. \e[39m"
            echo "Aborting script."
            echo "GUID=='29314cf2-64ea-4e42-84b5-93e0d0d1c7e7'"
            echo ""
            exit 1
        fi
    fi
...
func_mmmv_assert_environment_variable_set_t1() { 
    ...
    if [ "$S_ENVIR_VALUE" == "" ]; then
        ...
        echo "GUID=='57b0545c-9f25-4f1d-91b5-93e0d0d1c7e7'"
        echo ""
        exit 1 # exit with an error
    #else 
...
func_mmmv_iterate_over_array_02n_t1() {
    ...
    if [ "$S_ARRAY_VARIABLE_NAME" == "" ]; then
        ...
        echo "GUID=='c642a515-d1f1-4663-92a5-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_ITERATION_FUNCTION_NAME" == "" ]; then
        ...
        echo "GUID=='182abe34-3d95-4c87-94a5-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
...
func_mmmv_include_bashfile_if_possible_t1(){ # S_FP_BASHFILE S_GUID_CANDIDATE SB_THROW_ON_ERROR
    ...
    if [ "$SB_THROW_ON_ERROR" == "" ]; then
        ...
        if [ "$SB_THROW_ON_ERROR" != "t" ]; then
            if [ "$SB_THROW_ON_ERROR" != "f" ]; then
                ...
                echo "GUID=='38f1dd71-8e0e-45f0-92a5-93e0d0d1c7e7'"
                echo ""
            fi
        fi
    fi
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='fff8061f-cb09-427e-a3a5-93e0d0d1c7e7'"
        echo ""
        if [ "$SB_THROW_ON_ERROR" == "t" ]; then
            exit 1
        fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='3385f114-86c2-4ac5-94a5-93e0d0d1c7e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='d8165851-9566-4bfe-95a5-93e0d0d1c7e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='ba343c39-f87b-46dd-a4a5-93e0d0d1c7e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1 
            fi
...
func_mmmv_include_bashfile_if_possible_t2(){ # S_FP_BASHFILE S_GUID_CANDIDATE 
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='d63abf1f-6145-470d-93a5-93e0d0d1c7e7'"
        echo ""
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='08c6ba2f-9abd-4120-8195-93e0d0d1c7e7'"
            echo ""
        fi
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='e8ed9742-3b50-45f4-a595-93e0d0d1c7e7'"
            echo ""
        fi
    fi
    #----------------------------------------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='aa1cbf40-c3f9-49d4-a295-93e0d0d1c7e7'"
            echo ""
        fi
    fi
...
func_mmmv_assert_exists_on_path_t1() {
    ...
    if [ "$S_TMP_1" == "" ]; then
        ...
        echo "GUID=='f8a93c52-3424-44a5-b595-93e0d0d1c7e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_ln_create_or_overwrite_symlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ ! -e "$S_FP_LINK" ]; then
        ...
        echo ""
        echo "PWD==`pwd`"
        echo "GUID=='a033002c-78e0-4583-8595-93e0d0d1c7e7'"
        echo ""
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_determine_operatingsystem_t1() {
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "cec17115-9e2e-44fe-a337-93e0d0d1c7e7" 
    fi
...
func_mmmv_assert_Linux_or_BSD_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='36228335-20a1-41c9-b395-93e0d0d1c7e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" == "" ]; then
        ...
        echo ""
        echo "GUID=='a63f6234-0df0-4b10-b195-93e0d0d1c7e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
...
func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='cb627519-f58d-47a0-9195-93e0d0d1c7e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
            ...
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" != "f" ]; then
                ...
                if [ "$S_GUID_CANDIDATE" != "" ]; then
                    ...
                    func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1 \
                        "$SB_DISPLAY_NONERROR_FEEDBACK" \
                        "SB_DISPLAY_NONERROR_FEEDBACK" \
                        "0f0bad23-a5ab-4e93-8437-93e0d0d1c7e7"
                fi
                SB_DISPLAY_NONERROR_FEEDBACK="f"
                #----------------------------------------
            fi
...
# RENESSAATOR_BLOCK_END
    ...
    if [ "$SB_DOMAIN_IS_REACHABLE" == "f" ]; then
        ...
        if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            echo "GUID=='e3966f60-fba3-47c6-a495-93e0d0d1c7e7'"
            if [ "$S_GUID_CANDIDATE" != "" ]; then
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
        fi
...
func_mmmv_assert_internet_connection_exists_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='4265f524-dae9-4093-8185-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "ed93702e-e45a-4631-b127-93e0d0d1c7e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_STUDY_INTERNET_AVAILABILITY" == "t" ]; then
        ...
        # The 
        func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1 \
            "628df250-f4f9-470a-b417-93e0d0d1c7e7" \
            "$SB_DISPLAY_NONERROR_FEEDBACK"
        ...
        if [ "$SB_VERIFICATION_FAILED" == "t" ]; then
            ...
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
                ...
                func_mmmv_assert_sbvar_domain_t_f_t1 \
                    ...
                    "SB_VERIFICATION_FAILED" \
                    "4cd8cc75-793f-43de-af07-93e0d0d1c7e7"
            fi
        fi
        #----------------------------------------
    ...
    if [ "$SB_INTERNET_AVAILABLE" == "f" ]; then
        ...
        echo "GUID=='578eb82c-3c7a-4da2-a385-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_GUI_applications_can_be_launched_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='8620c05e-b443-4884-9185-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_OK_TO_CACHE" "SB_OK_TO_CACHE" \
            "fd352120-b61d-467f-a407-93e0d0d1c7e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_OK_TO_CACHE" == "t" ]; then
        if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_RESULT_CACHED" == "t" ]; then
            ...
            if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_GUI_IS_LAUNCHABLE" == "f" ]; then
                ...
                echo "Aborting script."
                echo "GUID=='ea2ed942-00b4-4c44-9285-93e0d0d1c7e7'"
                echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                echo ""
                #--------
                cd "$S_FP_ORIG"
    ...
    if [ "$SB_RUN_TEST" == "t" ]; then
        ...
        if [ "$S_TMP_0" != "124" ]; then
            ...
            echo "Aborting script."
            echo "GUID=='2aad0602-daa5-42f8-8b75-93e0d0d1c7e7'"
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_exc_initialize_wine_C_drive_if_needed_t1(){
    ...
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_DISPLAY_NONERROR_FEEDBACK" "SB_DISPLAY_NONERROR_FEEDBACK" \
            "8a96645a-62c9-414c-81e6-93e0d0d1c7e7" 
    fi
    #----------------------------------------------------------------------
    ...
    if [ ! -d "$S_FP_C" ]; then # for speed optimisation
        if [ ! -e "$S_FP_C" ]; then
            ...
            func_mmmv_assert_GUI_applications_can_be_launched_t1 \
                "1efe7e1a-b14d-42c1-b1d6-93e0d0d1c7e7" \
            ...
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "f2c69828-6695-4512-94d6-93e0d0d1c7e7"
            func_mmmv_wait_and_sync_t1
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "08300924-d96e-4bde-b5d6-93e0d0d1c7e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            #--------------------------------------------------------------
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "54fc12b4-62a8-4dc1-b2c6-93e0d0d1c7e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            #--------------------------------------------------------------
        fi
    fi
...
func_mmmv_uncompress_tar_dot_foo_and_enter_t1(){
    ...
    if [ "$SB_COMPRESSION_TYPE_RECOGNIZED" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='1f4a2d39-02ba-4415-a475-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_PARENT_FOLDER" \
        "9896d6e4-c619-48d1-a2c6-93e0d0d1c7e7"
    cd $S_FP_PARENT_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "375793d1-329a-4ebf-a2b6-93e0d0d1c7e7"
    #----------------------------------------
    ...
    if [ "$SB_COMPRESSIONLESS_TAR" == "f" ]; then
        func_mmmv_assert_file_exists_t1 "$S_FP_COMPRESSED" \
            "a80c9a4f-dedf-4eee-b5b6-93e0d0d1c7e7"
        $S_CMD_UNCOMPRESS $S_FP_COMPRESSED
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "56387644-0d5f-4879-a1b6-93e0d0d1c7e7"
        func_mmmv_wait_and_sync_t1
    fi
    #----------------------------------------
    func_mmmv_assert_file_exists_t1 "$S_FP_TAR" \
        "f2087b23-ead2-45b3-92a6-93e0d0d1c7e7"
    $S_CMD_NICE_PREFIX tar -xf $S_FP_TAR
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "54e20f41-43cc-468c-92a6-93e0d0d1c7e7"
    func_mmmv_wait_and_sync_t1
    #----------------------------------------
    func_mmmv_assert_folder_exists_t1 "$S_FP_NEXT_PARENT_FOLDER" \
        "b9ccfe40-905d-4802-b4a6-93e0d0d1c7e7"
...
func_mmmv_s_owner_of_a_file_or_folder_t1(){
    ...
    if [ ! -e "$S_FP_IN" ]; then
        if [ -h "$S_FP_IN" ]; then
            func_mmmv_exc_exit_with_an_error_t2 \
                "33d9b3f3-c9db-4c43-b596-93e0d0d1c7e7" \
                "The $S_FP_IN is a broken symlink."
        else
            func_mmmv_exc_exit_with_an_error_t2 \
                "5c733702-a1a6-4aa7-b796-93e0d0d1c7e7" \
...
func_mmmv_exc_block_device_ID_to_device_file_name_t1(){ # 
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='3eec0e1a-63b2-4f4a-b175-93e0d0d1c7e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_BLOCK_DEVICE_ID_SUBPART_CANDIDATE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='ad819c19-af65-4cde-b375-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='1c80cae1-b2b4-49ec-9375-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_FP_DEVICE_FILE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='e3a999ac-48f1-481d-b165-93e0d0d1c7e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_first_use_message(){
    ...
    echo "GUID=='73e80827-77f3-4c4e-8c65-93e0d0d1c7e7'"
    echo ""
    #------------------------------
    cd "$S_FP_ORIG"
    exit 1
...
func_display_help_message_and_exit_if_needed(){
    ...
    if [ "$S_ARGV_0" != "" ]; then
        if [ "`echo \"$S_ARGV_0\" | grep -E '^((([-][-]?)?[hH][eE][lL][pP])|([-]?[hH?]))$' `" == "" ]; then
            ...
            echo "GUID=='84bffc44-ecd5-4741-a365-93e0d0d1c7e7'"
...
func_main(){
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_DIR" \
        "51a9f76d-19a0-4fdd-ad86-93e0d0d1c7e7"
    cd $S_FP_DIR
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "7088fb1a-5db1-4a3f-8586-93e0d0d1c7e7"
    #------------------------------
    func_mmmv_create_folder_t1 "$S_FP_BUILD_FOLDER"
    cd $S_FP_BUILD_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "a2dac854-d8c2-49d5-a386-93e0d0d1c7e7"
    #------------------------------
    func_mmmv_assert_file_exists_t1 "$S_FP_SRC" \
        "c4e51e2b-36cb-4c3c-8476-93e0d0d1c7e7"
    ...
    cd $S_FP_PARENT_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "62fad838-fac2-4706-8176-93e0d0d1c7e7"
    $S_CMD_PREFIX_NICE ./ccnfigure --prefix=$S_FP_INSTALLATION_FOLDER 
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "6ff01255-ada5-4fdd-a166-93e0d0d1c7e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    ...
    #echo ""
    echo "Thank You for trying out this script. "
    echo "GUID=='718b3f59-7ec5-4c44-b165-93e0d0d1c7e7'"
    echo ""
    #------------------------------
} # func_main
func_main
...
exit 0
#==========================================================================
# S_VERSION_OF_THIS_FILE="3350485c-4ea4-4cbd-ac46-93e0d0d1c7e7"
#==========================================================================
