...
func_mmmv_bash_boilerplate_version(){
    local S_GUID="50cf7cc2-7ba4-4b9c-b932-a263a0e0a8e7"
...
func_mmmv_init_s_timestamp_if_not_inited_t1(){
    if [ "$S_TIMESTAMP" == "" ]; then
        if [ "`which date 2> /dev/null`" != "" ]; then
            ...
            echo ""
            echo "GUID=='29f7bb12-503a-4829-ad42-a263a0e0a8e7'"
            echo ""
        fi
    fi
} # func_mmmv_init_s_timestamp_if_not_inited_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1(){
    ...
    if [ "$S_FP_ORIG" == "" ]; then 
        ...
        echo "GUID=='4c25a9c3-8a4b-4d16-b522-a263a0e0a8e7'"
        echo ""
    else
        if [ ! -e "$S_FP_ORIG" ]; then 
            SB_S_FP_ORIG_VERIFICATION_FAILED="t"
            ...
            echo ""
            echo "GUID=='a5c0e633-f4dd-4ebf-9d4b-a263a0e0a8e7'"
            echo ""
        else
            if [ ! -d "$S_FP_ORIG" ]; then 
                SB_S_FP_ORIG_VERIFICATION_FAILED="t"
                ...
                echo "    S_FP_ORIG==$S_FP_ORIG"
                echo ""
                echo "GUID=='4b569695-a803-49a5-ae4a-a263a0e0a8e7'"
                echo ""
            fi
        fi
    fi
    ...
    #        echo "GUID=='3cbac381-d74f-4121-b34a-a263a0e0a8e7'"
    #        echo ""
    #    fi
} # func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t2(){
    if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo "GUID=='65d2e243-50b7-4992-9f29-a263a0e0a8e7'"
            echo ""
        else
            func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_assert_nonempty_string_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "GUID=='174ef9d8-102c-4218-9488-a263a0e0a8e7'"
        echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then 
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then 
            ...
            echo "GUID=='3da5fec3-c14c-4b5f-8258-a263a0e0a8e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='1d54a2c2-38b7-4652-aa57-a263a0e0a8e7'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
            ...
            echo ""
            echo "GUID=='52b5f627-13be-47cf-9117-a263a0e0a8e7'"
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_IN" == "" ]; then 
            ...
            echo "GUID=='7b60c882-3220-4c6c-9937-a263a0e0a8e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
...
func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='56c76931-f487-4c68-8444-a263a0e0a8e7'"
        echo ""
        SB_NO_ERRORS_YET_1="f"
    fi
    #--------------------
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then 
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then 
            echo "is expected to be initialized to \"t\" before calling this function."
            echo "GUID=='5e5afb42-9f18-4f22-8334-a263a0e0a8e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='21be0f65-89c1-4c55-8822-a263a0e0a8e7'"
        fi
        if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then 
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
            ...
            echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
            echo ""
            echo "GUID=='035add5c-1ad3-4665-a121-a263a0e0a8e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$SB_VARIABLE_VALUE" != "t" ]; then 
            if [ "$SB_VARIABLE_VALUE" != "f" ]; then 
                ...
                echo "GUID=='e2bd9a08-870c-4298-9921-a263a0e0a8e7'"
                if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                    echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                fi
                echo ""
...
func_mmmv_report_an_error_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "GUID=='31e76ae1-c5ea-4eaa-8820-a263a0e0a8e7'"
        echo ""
    else
        echo ""
        echo -e "\e[31mThe code of this script is flawed.\e[39m"
        ...
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='3b4a1cd4-d43e-40e4-a94f-a263a0e0a8e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because 
...
func_mmmv_report_missing_from_path_and_do_NOT_exit_t1() {
    ...
    if [ "$S_TMP_1" == "" ] ; then
        ...
        echo "GUID=='1994f992-d789-4d59-b12e-a263a0e0a8e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because 
...
func_mmmv_verify_that_the_file_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='265c3122-bb57-4329-9f5e-a263a0e0a8e7'"
        echo ""
        #--------
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='7b112a6b-3bf4-4b2a-937d-a263a0e0a8e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID=='3f29e523-514c-48ad-be5d-a263a0e0a8e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='49672d95-9367-4880-941c-a263a0e0a8e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='e33f5da9-4fd0-4481-83ac-a263a0e0a8e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            ...
            echo "GUID=='398374c5-4025-47b9-b34b-a263a0e0a8e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "This function requires 2 parameters: S_FP, S_GUID_CANDIDATE"
        echo "and has an optional 3. parameter: SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE"
        echo "GUID=='92aeb2fd-f33c-41b3-bbda-a263a0e0a8e7'"
        echo ""
        #--------
        SB_VERIFICATION_FAILED="t"
    fi
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "Valid values are: \"t\", \"f\", \"\" ."
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='f0b9c9e3-c0fb-4e63-a919-a263a0e0a8e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='41946e61-45fa-490c-a527-a263a0e0a8e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo -e "\e[31mdoes not exist\e[39m."
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='f43ea35f-bf61-4221-8b47-a263a0e0a8e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ ! -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='505aff12-5ad3-4dc6-9947-a263a0e0a8e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='5ff9d6e4-dc8f-4203-8b45-a263a0e0a8e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_add_bin_2_Z_PATH_and_optionally_share_man_2_MANPATH_t1(){
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_GUID_CANDIDATE" == "" ]; then
            ...
            echo "    S_GUID_CANDIDATE==\"\""
            echo ""
            echo "GUID=='1c934523-d5dd-4e32-b035-a263a0e0a8e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_FP_INSTALLATION_FOLDER" == "" ]; then
            ...
            echo "GUID=='3f089843-df21-4703-9e33-a263a0e0a8e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
        ...
        if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "" ]; then
            ...
            if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "t" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "f" ]; then
                    ...
                    echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                    echo "GUID=='849f434f-d506-4f8b-ad13-a263a0e0a8e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
            "$S_FP_INSTALLATION_FOLDER" "35cbd525-3953-479c-963d-a263a0e0a8e7"
        if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            #--------------
            func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                "$S_FP_INSTALLATION_FOLDER/bin" "1b1cf874-9696-45d6-9a1b-a263a0e0a8e7"
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            ...
            if [ "$SB_MAN_FOLDER_OR_NONBROKEN_SYMLINK_TO_IT_EXISTS" == "f" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "t" ]; then
                    ...
                    func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                        "$S_FP_INSTALLATION_FOLDER/share/man" "36854354-ae84-492d-984a-a263a0e0a8e7"
                fi
            else
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='379e9174-6c4d-45f9-b841-a263a0e0a8e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_assert_error_code_zero_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='632f2c8e-e41e-42cb-8041-a263a0e0a8e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "GUID=='8b4cbbfc-493e-46f3-bf30-a263a0e0a8e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_error_code_zero_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='e7ac10ce-94b1-448d-a291-a263a0e0a8e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "Something went wrong. Error code: $S_ERR_CODE"
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='b5850c37-9393-4f7b-ba39-a263a0e0a8e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
...
func_mmmv_exc_verify_S_FP_ORIG_t1() {
    if [ "$S_FP_ORIG" == "" ]; then
        ...
        echo ""
        echo "Aborting script."
        echo "GUID=='931561e9-3c6c-4f22-b4c1-a263a0e0a8e7'"
        echo ""
    ...
    if [ ! -e "$S_FP_ORIG" ]; then
        ...
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='8239ee4f-12c0-4a16-9d2d-a263a0e0a8e7'"
        echo ""
        exit 1 # exit with an error
    fi
    #------------------------
    if [ ! -d "$S_FP_ORIG" ]; then
        ...
        echo "contains the script that prints this error message."
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='ffe41796-3555-4ccb-bfe2-a263a0e0a8e7'"
        echo ""
        exit 1 # exit with an error
    fi
} # func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_exc_verify_S_FP_ORIG_t2(){
    if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo ""
            echo "has a domain of {\"f\", \"t\"}."
            echo "GUID=='4d5342b3-ba13-424d-b661-a263a0e0a8e7'"
            echo ""
        else
            func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_cd_S_FP_ORIG_and_exit_t1(){
    ...
    cd "$S_FP_ORIG"
    func_mmmv_assert_error_code_zero_t2 "$?" \
        "4b302eb5-693e-4cae-8849-a263a0e0a8e7"
    exit 0
} # func_mmmv_cd_S_FP_ORIG_and_exit_t1
...
func_mmmv_assert_nonempty_string_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='4617738f-33c7-4516-8011-a263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='ffa23a8d-379d-4586-a9b1-a263a0e0a8e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_IN" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be something other than an empty string."
        echo "GUID=='132086cc-87d3-43ef-bcd2-a263a0e0a8e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_sbvar_domain_t_f_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='e25aba86-0e63-445e-8284-a263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='7c859bc7-22b9-48dc-b2e4-a263a0e0a8e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$SB_VARIABLE_VALUE" != "t" ]; then 
        if [ "$SB_VARIABLE_VALUE" != "f" ]; then 
            ...
            echo ""
            echo "but it is expected to be either \"t\" or \"f\"."
            echo "GUID=='446336a7-d637-432f-8a34-a263a0e0a8e7'"
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_assert_file_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='57e31a6e-c426-4a29-b5aa-a263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            ...
            echo "GUID=='51e37a9a-a9c4-4395-b0b1-a263a0e0a8e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='b388e462-c5c4-4d8b-8e2e-a263a0e0a8e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='e59bef04-7c51-44d4-bd63-a263a0e0a8e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='2666eece-4f74-42e2-b692-a263a0e0a8e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='386cf97d-ac23-44c1-8884-a263a0e0a8e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='49569751-5090-4f1e-9d01-a263a0e0a8e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_assert_folder_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='0309ae9d-0a12-44c4-aef4-a263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            echo -e "\e[31mThis code is flawed. \e[39m"
            echo "GUID=='94d60df5-5d2c-4146-8031-a263a0e0a8e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='e841d13e-0822-4591-b0b4-a263a0e0a8e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='03c3f90f-7178-41ef-bc75-a263a0e0a8e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='518e121c-aeec-4390-a2c1-a263a0e0a8e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ ! -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='65f4034b-4f4d-4498-9f23-a263a0e0a8e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='9489177c-fcdc-4385-bd95-a263a0e0a8e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_exit_if_not_on_path_t2() { # S_COMMAND_NAME
    ...
    if [ "$S_LOCAL_VARIABLE" == "" ]; then
        ...
        echo "GUID=='c628e579-47a9-4326-af29-a263a0e0a8e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1;
    fi
...
func_mmmv_exc_exit_with_an_error_t1(){
    ...
    echo "GUID=='b5621e49-da3e-4470-b005-a263a0e0a8e7'"
    echo ""
    cd "$S_FP_ORIG"
    exit 1 # exit with an error
} # func_mmmv_exc_exit_with_an_error_t1
...
func_mmmv_exc_exit_with_an_error_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "Aborting script."
        echo "GUID=='f483a37c-8b5a-45ed-ab02-a263a0e0a8e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    else
        ...
        echo "Aborting script."
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='94a03994-abc7-41b0-a272-a263a0e0a8e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_determine_Awk_command_t1() { # prefers the GNU Awk to the BSD Awk
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "d771b8d3-d60c-4604-8c28-a263a0e0a8e7" 
    ...
    if [ "$SB_REINIT" == "t" ]; then
        ...
        local SB_THROW="f"
        local S_TMP_0="1acd75b4-e21a-4cc9-9b58-a263a0e0a8e7"
        ...
        if [ "$SB_THROW" == "t" ]; then
            ...
            echo "The execution of this Bash script is aborted."
            echo "GUID=='626179a5-e5a0-4944-8661-a263a0e0a8e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1;
        fi
...
func_mmmv_exc_determine_sed_command_t1() { # prefers the GNU sed to the BSD sed
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "5df8cb65-5bd8-4e47-aa48-a263a0e0a8e7" 
        SB_AVOID_REINIT="$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED"
    fi
    #----------------------------------------------------------------------
    local SB_REINIT="t"
    ...
    if [ "$SB_REINIT" == "t" ]; then
        ...
        local SB_THROW="f"
        local S_TMP_0="4becefd3-05c2-4dae-8917-a263a0e0a8e7"
        local S_FP_0="/tmp/this/proB9bably/do00es/noTt/eXXXist/ㄫ/$S_TMP_0"
        #------------------------------------------------------------------
        ...
        if [ "$SB_THROW" != "f" ]; then
            ...
            echo "The execution of this Bash script is aborted."
            echo "GUID=='3cfa3035-32cc-41c2-9fc2-a263a0e0a8e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1;
        fi
...
func_mmmv_exc_is_file_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then 
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='724f2e80-f3c1-4479-b703-a263a0e0a8e7'"
    ...
    if [ -d "$S_FP_CANDIDATE" ]; then 
        ...
        echo "GUID=='61523f8f-2f13-4e12-99b7-a263a0e0a8e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_is_folder_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then 
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='456d0ab2-9da5-419c-88e3-a263a0e0a8e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
    if [ ! -d "$S_FP_CANDIDATE" ]; then 
        ...
        echo "GUID=='c631ca9c-24b1-4b82-8053-a263a0e0a8e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_create_folder_t1(){
    ...
    if [ "$S_FP_FOLDER" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='3523b1ca-891a-4485-828e-9263a0e0a8e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
    ...
    if [ -e "$S_FP_FOLDER" ]; then 
        if [ ! -d "$S_FP_FOLDER" ]; then 
            ...
            echo "Aborting script."
            echo "GUID=='4050a695-8280-4c04-928e-9263a0e0a8e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1 # exit with an error
        fi
        ...
        wait # just in case
        if [ "$S_TMP_0" != "0" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "217f6505-15a7-4459-8d17-a263a0e0a8e7" \
                "S_FP_FOLDER==$S_FP_FOLDER"
        fi
        if [ ! -e "$S_FP_FOLDER" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "2426a701-b73e-495b-b154-a263a0e0a8e7" \
...
func_mmmv_exc_assure_tmp_folder_existence_t1() {  # S_FP_TMP, S_GUID
    ...
    if [ "$S_GUID" == "" ]; then
        ...
        echo "GUID=='1560c317-05d8-4779-b67e-9263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP_TMP" ]; then
        if [ -h "$S_FP_TMP" ]; then
            ...
            func_mmmv_assert_error_code_zero_t1 \
                "$?" "4b305d14-a0a9-41ca-97a4-a263a0e0a8e7"
            func_mmmv_wait_and_sync_t1
            if [ -h "$S_FP_TMP" ]; then
                echo ""
                echo "The path "
                ...
                echo "GUID==\"$S_GUID\""
                echo "GUID=='24d43b53-754c-4b20-927e-9263a0e0a8e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
        ...
        mkdir -p "$S_FP_TMP"
        func_mmmv_assert_error_code_zero_t1 \
            "$?" "423ded09-7a42-4e59-8c23-a263a0e0a8e7"
        func_mmmv_wait_and_sync_t1
        func_mmmv_assert_folder_exists_t1 "$S_FP_TMP" \
            '19198711-53c8-40fa-b37e-9263a0e0a8e7'
    else
        if [ ! -d "$S_FP_TMP" ]; then
            echo ""
            if [ -h "$S_FP_TMP" ]; then
            ...
            echo "GUID==\"$S_GUID\""
            echo "GUID=='70689d4f-b1be-4549-847e-9263a0e0a8e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
...
func_mmmv_ln_create_hardlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ "$SB_THROW_ON_INVALID_DATA" == "" ]; then
        ...
        if [ "$SB_THROW_ON_INVALID_DATA" != "t" ]; then
            if [ "$SB_THROW_ON_INVALID_DATA" != "f" ]; then
                ...
                echo ""
                echo "Aborting script."
                echo "GUID=='8b336b38-ac11-4acf-947e-9263a0e0a8e7'"
                echo ""
    ...
    if [ -e "$S_FP_LINK" ]; then 
        if [ -d "$S_FP_LINK" ]; then # folder or a symlink to a folder
            if [ ! -h "$S_FP_LINK" ]; then # not a symlink, therefore a folder
                ...
                echo "    $S_FP_TARGET"
                echo ""
                echo "GUID=='5ec04d8d-a324-437e-916e-9263a0e0a8e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            echo "GUID=='25e4e6ec-6862-42d9-856e-9263a0e0a8e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            if [ -h "$S_FP_LINK" ]; then # a broken symlink, therefore NOT missing
                ...
                echo "must not exist before the call to this function."
                echo "GUID=='8e15392b-dc51-4114-b26e-9263a0e0a8e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ -e "$S_FP_TARGET" ]; then
            if [ -d "$S_FP_TARGET" ]; then # a folder or a symlink to a folder
                if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore a folder
                    ...
                    echo "    $S_FP_LINK"
                    echo ""
                    echo "GUID=='5e1b4ce3-84a5-4f8b-a36e-9263a0e0a8e7'"
                    echo ""
                    if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                        exit 1
                    fi
            ...
            if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore missing
                ...
                echo "    $S_FP_LINK"
                echo ""
                echo "GUID=='bdda3a5f-c6d1-4e29-946e-9263a0e0a8e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
    ...
    #----------------------------------------------------------------------
    local S_TMP_0="not_set_yet GUID=='9ab28e12-d2fd-4ace-a36e-9263a0e0a8e7'"
    if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
        ...
        if [ "$S_TMP_0" != "0" ]; then
            ...
            echo "GUID=='cd97e652-1871-4d55-b35e-9263a0e0a8e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            if [ -h "$S_FP_TARGET" ]; then
                ...
                if [ ! -h "$S_FP_LINK" ]; then
                    ...
                    echo "GUID=='2310145a-6818-4464-915e-9263a0e0a8e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -e "$S_FP_TARGET" ]; then # symlink to a folder or a file
                        if [ ! -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='20526e1b-231a-4a7a-835e-9263a0e0a8e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ -d "$S_FP_TARGET" ]; then
                                if [ ! -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='25135105-28ec-4132-b25e-9263a0e0a8e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='03eeac3e-1f86-44bc-a34e-9263a0e0a8e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
                        ...
                        if [ -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='5ff11884-104c-46e7-b54e-9263a0e0a8e7'"
                            echo ""
                            S_TMP_0="t"
                        fi
                    fi
                ...
                if [ ! -e "$S_FP_TARGET" ]; then # just an extra test
                    echo ""
                    echo "Problem detection branch marker "
                    echo "GUID=='30f68051-92b0-4781-814e-9263a0e0a8e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -d "$S_FP_TARGET" ]; then # just an extra test
                        echo ""
                        echo "Problem detection branch marker "
                        echo "GUID=='f7f6e0f6-e500-4572-924e-9263a0e0a8e7'"
                        echo ""
                        S_TMP_0="t"
                    else
                        if [ -h "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='6d6e545c-ad05-46b4-814e-9263a0e0a8e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ ! -e "$S_FP_LINK" ]; then
                                echo ""
                                echo "Problem detection branch marker "
                                echo "GUID=='948ad710-bd02-42d1-933e-9263a0e0a8e7'"
                                echo ""
                                S_TMP_0="t"
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='3ec91938-b8bf-42d9-a33e-9263a0e0a8e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
            ...
            if [ "$S_TMP_0" == "t" ]; then
                ...
                echo "GUID=='751d4710-e055-40fc-933e-9263a0e0a8e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
                ...
                if [ "$S_TMP_0" != "f" ]; then
                    ...
                    echo "Aborting script."
                    echo "GUID=='8a724e16-e824-4dee-b23e-9263a0e0a8e7'"
                    echo ""
                    exit 1
                fi
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" != "t" ]; then
            ...
            echo -e "\e[31mThe implementation of this function is flawed. \e[39m"
            echo "Aborting script."
            echo "GUID=='889fac1a-44a3-42d9-933e-9263a0e0a8e7'"
            echo ""
            exit 1
        fi
    fi
...
func_mmmv_assert_environment_variable_set_t1() { 
    ...
    if [ "$S_ENVIR_VALUE" == "" ]; then
        ...
        echo "GUID=='2232e939-c5b6-4115-942e-9263a0e0a8e7'"
        echo ""
        exit 1 # exit with an error
    #else 
...
func_mmmv_iterate_over_array_02n_t1() {
    ...
    if [ "$S_ARRAY_VARIABLE_NAME" == "" ]; then
        ...
        echo "GUID=='1ae41b40-ce2c-4f05-b32e-9263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_ITERATION_FUNCTION_NAME" == "" ]; then
        ...
        echo "GUID=='7f53563d-a262-4c8c-b22e-9263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
...
func_mmmv_include_bashfile_if_possible_t1(){ # S_FP_BASHFILE S_GUID_CANDIDATE SB_THROW_ON_ERROR
    ...
    if [ "$SB_THROW_ON_ERROR" == "" ]; then
        ...
        if [ "$SB_THROW_ON_ERROR" != "t" ]; then
            if [ "$SB_THROW_ON_ERROR" != "f" ]; then
                ...
                echo "GUID=='97578e31-86cc-4e70-b42e-9263a0e0a8e7'"
                echo ""
            fi
        fi
    fi
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='1905404c-9fbb-450b-812e-9263a0e0a8e7'"
        echo ""
        if [ "$SB_THROW_ON_ERROR" == "t" ]; then
            exit 1
        fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='3603d638-a36e-4a9a-951e-9263a0e0a8e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='2a85b715-63f6-4802-b51e-9263a0e0a8e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='fa083028-59a4-4010-921e-9263a0e0a8e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1 
            fi
...
func_mmmv_include_bashfile_if_possible_t2(){ # S_FP_BASHFILE S_GUID_CANDIDATE 
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='49c55154-7c98-45bf-b31e-9263a0e0a8e7'"
        echo ""
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='c72c5a2d-6894-455f-b21e-9263a0e0a8e7'"
            echo ""
        fi
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='a2050e2b-2f5c-47db-910e-9263a0e0a8e7'"
            echo ""
        fi
    fi
    #----------------------------------------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='9309fcc7-3ef5-4281-910e-9263a0e0a8e7'"
            echo ""
        fi
    fi
...
func_mmmv_assert_exists_on_path_t1() {
    ...
    if [ "$S_TMP_1" == "" ]; then
        ...
        echo "GUID=='14543cf4-4533-47ee-b60e-9263a0e0a8e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_ln_create_or_overwrite_symlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ -e "$S_FP_LINK" ]; then
        if [ ! -h "$S_FP_LINK" ]; then # not a symlink
            ...
            echo ""
            echo "PWD==`pwd`"
            echo "GUID=='7ab4f464-0b67-4776-830e-9263a0e0a8e7'"
            echo ""
            exit 1 # exit with an error
        fi
    ...
    if [ "$SB_DELETE_OLD_LINK" == "t" ]; then
        rm -f $S_FP_LINK
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "f2d4cd6b-3d3c-4327-8c32-a263a0e0a8e7"
        func_mmmv_wait_and_sync_t1
        if [ -e "$S_FP_LINK" ]; then
            echo ""
            echo ""
            ...
            echo ""
            echo "PWD==`pwd`"
            echo "GUID=='a7151951-66a3-4f81-b20e-9263a0e0a8e7'"
            echo ""
            echo ""
            exit 1 # exit with an error
        fi
    ...
    $S_CMD_LN
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "110cece9-7514-4389-a941-a263a0e0a8e7"
    func_mmmv_wait_and_sync_t1
    if [ ! -e "$S_FP_LINK" ]; then
        echo ""
        echo ""
        ...
        echo ""
        echo "PWD==`pwd`"
        echo "GUID=='150a403e-3014-4ed6-b5fd-9263a0e0a8e7'"
        echo ""
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_determine_operatingsystem_t1() {
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "da08bda5-f449-4c12-a410-a263a0e0a8e7" 
    fi
...
func_mmmv_assert_Linux_or_BSD_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='6b9d6a28-40b0-4b35-a1fd-9263a0e0a8e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" == "" ]; then
        ...
        echo ""
        echo "GUID=='fbc19234-09f1-4df8-81fd-9263a0e0a8e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
...
func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='edddbd37-2901-4f76-b3fd-9263a0e0a8e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
            ...
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" != "f" ]; then
                ...
                if [ "$S_GUID_CANDIDATE" != "" ]; then
                    ...
                    func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1 \
                        "$SB_DISPLAY_NONERROR_FEEDBACK" \
                        "SB_DISPLAY_NONERROR_FEEDBACK" \
                        "3d1cd3b5-9997-4415-ba50-a263a0e0a8e7"
                fi
                SB_DISPLAY_NONERROR_FEEDBACK="f"
                #----------------------------------------
            fi
...
# RENESSAATOR_BLOCK_END
    ...
    if [ "$SB_DOMAIN_IS_REACHABLE" == "f" ]; then
        ...
        if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            echo "GUID=='728cf12e-1411-44c1-91ed-9263a0e0a8e7'"
            if [ "$S_GUID_CANDIDATE" != "" ]; then
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
        fi
...
func_mmmv_assert_internet_connection_exists_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='22ed3493-ebad-470e-83ed-9263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "23c45e22-7022-4353-af2e-a263a0e0a8e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_STUDY_INTERNET_AVAILABILITY" == "t" ]; then
        ...
        # The 
        func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1 \
            "4fd90713-c548-41fb-a65d-a263a0e0a8e7" \
            "$SB_DISPLAY_NONERROR_FEEDBACK"
        ...
        if [ "$SB_VERIFICATION_FAILED" == "t" ]; then
            ...
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
                ...
                func_mmmv_assert_sbvar_domain_t_f_t1 \
                    ...
                    "SB_VERIFICATION_FAILED" \
                    "639b0a84-52db-4574-944c-a263a0e0a8e7"
            fi
        fi
        #----------------------------------------
    ...
    if [ "$SB_INTERNET_AVAILABLE" == "f" ]; then
        ...
        echo "GUID=='05715da2-cbe2-491f-a3ed-9263a0e0a8e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_GUI_applications_can_be_launched_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='e4a7581f-261e-4e3b-b1ed-9263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_OK_TO_CACHE" "SB_OK_TO_CACHE" \
            "e49d5ce1-f991-43a4-a7cb-a263a0e0a8e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_OK_TO_CACHE" == "t" ]; then
        if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_RESULT_CACHED" == "t" ]; then
            ...
            if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_GUI_IS_LAUNCHABLE" == "f" ]; then
                ...
                echo "Aborting script."
                echo "GUID=='25305511-909d-41d3-b6ed-9263a0e0a8e7'"
                echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                echo ""
                #--------
                cd "$S_FP_ORIG"
    ...
    if [ "$SB_RUN_TEST" == "t" ]; then
        ...
        if [ "$S_TMP_0" != "124" ]; then
            ...
            echo "Aborting script."
            echo "GUID=='ad70084f-2acd-4eca-b3dd-9263a0e0a8e7'"
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_exc_initialize_wine_C_drive_if_needed_t1(){
    ...
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_DISPLAY_NONERROR_FEEDBACK" "SB_DISPLAY_NONERROR_FEEDBACK" \
            "63e90e06-6342-45c0-a217-a263a0e0a8e7" 
    fi
    #----------------------------------------------------------------------
    ...
    if [ ! -d "$S_FP_C" ]; then # for speed optimisation
        if [ ! -e "$S_FP_C" ]; then
            ...
            func_mmmv_assert_GUI_applications_can_be_launched_t1 \
                "58039e91-199b-4072-9b45-a263a0e0a8e7" \
            ...
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "011c538f-5be2-4297-a2a5-a263a0e0a8e7"
            func_mmmv_wait_and_sync_t1
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "94478937-d3ce-4158-b943-a263a0e0a8e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            #--------------------------------------------------------------
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "2c79fb93-822c-4c34-b423-a263a0e0a8e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            #--------------------------------------------------------------
        fi
    fi
...
func_mmmv_uncompress_tar_dot_foo_and_enter_t1(){
    ...
    if [ "$SB_COMPRESSION_TYPE_RECOGNIZED" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='3fc2e09d-201c-4e96-92dd-9263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_PARENT_FOLDER" \
        "0b146c2f-f8b1-46e3-8f51-a263a0e0a8e7"
    cd $S_FP_PARENT_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "c3523d15-43fa-4a3f-8610-a263a0e0a8e7"
    #----------------------------------------
    ...
    if [ "$SB_COMPRESSIONLESS_TAR" == "f" ]; then
        func_mmmv_assert_file_exists_t1 "$S_FP_COMPRESSED" \
            "36716d84-5008-495b-bc30-a263a0e0a8e7"
        $S_CMD_UNCOMPRESS $S_FP_COMPRESSED
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "94879f11-c117-43ab-bee1-a263a0e0a8e7"
        func_mmmv_wait_and_sync_t1
    fi
    #----------------------------------------
    func_mmmv_assert_file_exists_t1 "$S_FP_TAR" \
        "91877678-2133-43ae-8c32-a263a0e0a8e7"
    $S_CMD_NICE_PREFIX tar -xf $S_FP_TAR
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "b4b221ec-93a2-423a-94a1-a263a0e0a8e7"
    func_mmmv_wait_and_sync_t1
    #----------------------------------------
    func_mmmv_assert_folder_exists_t1 "$S_FP_NEXT_PARENT_FOLDER" \
        "65db4886-584e-428a-b145-a263a0e0a8e7"
...
func_mmmv_s_owner_of_a_file_or_folder_t1(){
    ...
    if [ ! -e "$S_FP_IN" ]; then
        if [ -h "$S_FP_IN" ]; then
            func_mmmv_exc_exit_with_an_error_t2 \
                "31e5f65c-233b-47f1-bf72-a263a0e0a8e7" \
                "The $S_FP_IN is a broken symlink."
        else
            func_mmmv_exc_exit_with_an_error_t2 \
                "e144821d-ccaf-41ed-90f5-a263a0e0a8e7" \
...
func_mmmv_exc_block_device_ID_to_device_file_name_t1(){ # 
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='fcf9dd22-618b-4abc-a2dd-9263a0e0a8e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_BLOCK_DEVICE_ID_SUBPART_CANDIDATE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='994af32f-43c8-4479-81dd-9263a0e0a8e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='161efc11-0705-4a04-b3dd-9263a0e0a8e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_FP_DEVICE_FILE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='3316f251-6f8e-4450-a3dd-9263a0e0a8e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_delete_folder_of_build_folders(){
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_FOLDER_OF_BUILD_FOLDERS" \
        "9135f546-c00f-4211-9c03-a263a0e0a8e7" \
        "$SB_OPTIONAL_BAN_SYMLINKS"
    #------------------------------
    ...
    if [ -e "$S_FP_DELETABLE" ]; then
        ...
        echo ""
        echo "GUID=='0e514359-d1bb-4431-b3dd-9263a0e0a8e7'"
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
        exit 1
        ...
        if [ -h "$S_FP_DELETABLE" ]; then # a broken symlink here
            ...
            echo "    S_FP_DELETABLE==\"$S_FP_DELETABLE\""
            echo ""
            echo "GUID=='a6fe4a15-4304-4da5-84dd-9263a0e0a8e7'"
            echo ""
            #------------------------------
            cd "$S_FP_ORIG"
            exit 1
    ...
    if [ "$S_TMP_0" != "" ]; then # for safety
        ...
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "f28b3dcf-f4f9-4177-8305-a263a0e0a8e7"
        func_mmmv_wait_and_sync_t1
        ...
        echo ""
        echo "GUID=='424b0517-095a-4cfa-94cd-9263a0e0a8e7'"
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
        exit 1
...
func_first_use_message(){
    ...
    echo "GUID=='53fe62b9-385e-46a5-a2cd-9263a0e0a8e7'"
    echo ""
    #------------------------------
    cd "$S_FP_ORIG"
    exit 1
...
func_display_help_message_and_exit_if_needed(){
    ...
    if [ "$S_ARGV_0" != "" ]; then
        if [ "`echo \"$S_ARGV_0\" | grep -E '^((([-][-]?)?[hH][eE][lL][pP])|([-]?[hH?]))$' `" == "" ]; then
            ...
            echo "GUID=='54eea51d-49c4-4425-83cd-9263a0e0a8e7'"
...
func_initialize_configuration_by_initializing_global_variables(){
    ...
    S_FP_INSTALLATION_FOLDER="`cd $S_FP_DIR/../../; pwd `/v_5_1_0"
    S_FN_BASENAME="worker-5.1.0"
...
func_main(){
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_DIR" \
        "a13f4fda-771d-4493-9904-a263a0e0a8e7"
    cd $S_FP_DIR
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "95a003ca-5e1b-45cd-a6e3-a263a0e0a8e7"
    #------------------------------
    func_mmmv_create_folder_t1 "$S_FP_BUILD_FOLDER"
    cd $S_FP_BUILD_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "96c180c4-1e3e-4c4c-8759-a263a0e0a8e7"
    #------------------------------
    func_mmmv_assert_file_exists_t1 "$S_FP_SRC" \
        "53f014e6-f401-44f4-8bd1-a263a0e0a8e7"
    $S_CMD_PREFIX_NICE cp -f $S_FP_SRC $S_FP_BUILD_FOLDER/
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "7f75a129-a4ba-4f6b-a9e5-a263a0e0a8e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    ...
    if [ -e "$S_FP_INSTALLATION_FOLDER" ]; then
        S_TMP_0="`echo \"$S_FP_INSTALLATION_FOLDER\" | grep -E '/v_5_1_0$' `"
        if [ "$S_TMP_0" != "" ]; then # for safety
            func_mmmv_assert_folder_exists_t1 "$S_FP_INSTALLATION_FOLDER" \
                "23dd6e3a-32db-453a-9005-a263a0e0a8e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            ...
            echo ""
            echo "GUID=='f2b15262-78bf-4108-bebd-9263a0e0a8e7'"
            echo ""
            #------------------------------
            cd "$S_FP_ORIG"
            exit 1
    ...
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "34df61c4-34b0-4160-8c54-a263a0e0a8e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    func_delete_folder_of_build_folders
    #------------------------------
    ...
    echo ""
    echo "Thank You for trying out this script. "
    echo "GUID=='0ef1c017-6a81-49f2-a2bd-9263a0e0a8e7'"
    echo ""
    #------------------------------
} # func_main
func_main
...
exit 0
#==========================================================================
# S_VERSION_OF_THIS_FILE="f21d532b-c6c2-422b-8ba4-a263a0e0a8e7"
#==========================================================================
