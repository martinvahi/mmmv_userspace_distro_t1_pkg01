...
func_mmmv_bash_boilerplate_version(){
    local S_GUID="1d7b961a-37ae-4eab-b3d6-1292908139e7"
...
func_mmmv_init_s_timestamp_if_not_inited_t1(){
    if [ "$S_TIMESTAMP" == "" ]; then
        if [ "`which date 2> /dev/null`" != "" ]; then
            ...
            echo ""
            echo "GUID=='e1f1c523-f879-418b-93d6-1292908139e7'"
            echo ""
        fi
    fi
} # func_mmmv_init_s_timestamp_if_not_inited_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1(){
    ...
    if [ "$S_FP_ORIG" == "" ]; then 
        ...
        echo "GUID=='3f05ee9f-21d5-4785-a2d6-1292908139e7'"
        echo ""
    else
        if [ ! -e "$S_FP_ORIG" ]; then 
            SB_S_FP_ORIG_VERIFICATION_FAILED="t"
            ...
            echo ""
            echo "GUID=='b30d5742-46b2-40a7-b5d6-1292908139e7'"
            echo ""
        else
            if [ ! -d "$S_FP_ORIG" ]; then 
                SB_S_FP_ORIG_VERIFICATION_FAILED="t"
                ...
                echo "    S_FP_ORIG==$S_FP_ORIG"
                echo ""
                echo "GUID=='8367f022-9e28-492d-93c6-1292908139e7'"
                echo ""
            fi
        fi
    fi
    ...
    #        echo "GUID=='3bb19db1-0a52-441e-bac6-1292908139e7'"
    #        echo ""
    #    fi
} # func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t2(){
    if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$SB_FUNC_MMMV_VERIFY_S_FP_ORIG_BUT_DO_NOT_EXIT_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo "GUID=='25a15391-8831-4a83-81c6-1292908139e7'"
            echo ""
        else
            func_mmmv_verify_S_FP_ORIG_but_do_not_exit_t1
...
func_mmmv_assert_nonempty_string_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "GUID=='168a491d-b156-4851-85c6-1292908139e7'"
        echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then 
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then 
            ...
            echo "GUID=='8c78b846-f1b5-4c50-b1c6-1292908139e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='20383c18-8bff-4b4f-95b6-1292908139e7'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
            ...
            echo ""
            echo "GUID=='3e7d3c75-05fb-4348-a266-1292908139e7'"
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_IN" == "" ]; then 
            ...
            echo "GUID=='c1e65b2b-39b4-41d8-b566-1292908139e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
...
func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='9fb2ec27-a169-49ff-9366-1292908139e7'"
        echo ""
        SB_NO_ERRORS_YET_1="f"
    fi
    #--------------------
    if [ "$SB_NO_ERRORS_YET" != "t" ]; then 
        ...
        if [ "$SB_NO_ERRORS_YET" == "f" ]; then 
            echo "is expected to be initialized to \"t\" before calling this function."
            echo "GUID=='ee831d3c-bd0e-4708-8156-1292908139e7'"
        else
            echo "is expected to be initialized to \"t\" before calling this function"
            echo "and its domain is {\"f\", \"t\"}."
            echo "GUID=='f93b063d-12a0-436c-b146-1292908139e7'"
        fi
        if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then 
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        fi
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
            ...
            echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
            echo ""
            echo "GUID=='27b8ae18-3534-4d29-a546-1292908139e7'"
            if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
            echo ""
    ...
    if [ "$SB_NO_ERRORS_YET" == "t" ]; then 
        if [ "$SB_VARIABLE_VALUE" != "t" ]; then 
            if [ "$SB_VARIABLE_VALUE" != "f" ]; then 
                ...
                echo "GUID=='64d19615-86d6-4238-a246-1292908139e7'"
                if [ "$SB_NO_ERRORS_YET_1" == "t" ]; then  # should be always true at this line
                    echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                fi
                echo ""
...
func_mmmv_report_an_error_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "GUID=='6eb2b252-226a-4016-b536-1292908139e7'"
        echo ""
    else
        echo ""
        echo -e "\e[31mThe code of this script is flawed.\e[39m"
        ...
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='40898cc3-78ce-420d-9f36-1292908139e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because 
...
func_mmmv_report_missing_from_path_and_do_NOT_exit_t1() {
    ...
    if [ "$S_TMP_1" == "" ] ; then
        ...
        echo "GUID=='812e1dad-1122-4424-a126-1292908139e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
    # exit 1 # must NOT be called in ~/.bashrc, because 
...
func_mmmv_verify_that_the_file_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='79c2bf48-156f-42e1-b216-1292908139e7'"
        echo ""
        #--------
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='12abd55e-11d2-4e84-9316-1292908139e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID=='93b03b16-e23b-4832-b206-1292908139e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='68b1dda4-c22e-4383-b406-1292908139e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='10d9c8a2-5772-4b98-8206-1292908139e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            ...
            echo "GUID=='c64aff13-03a1-4e28-a3f5-1292908139e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1() {  # S_FP, S_GUID_CANDIDATE
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "This function requires 2 parameters: S_FP, S_GUID_CANDIDATE"
        echo "and has an optional 3. parameter: SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE"
        echo "GUID=='47b1fe61-eea2-411f-94f5-1292908139e7'"
        echo ""
        #--------
        SB_VERIFICATION_FAILED="t"
    fi
        ...
        if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "" ]; then
            if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" == "f" ]; then
                ...
                if [ "$SB_DISPLAY_VERIFICATION_FAILURE_MESSAGE" != "t" ]; then
                    ...
                    echo "Valid values are: \"t\", \"f\", \"\" ."
                    echo "\"\" defaults to \"t\"."
                    echo "GUID=='508bc177-fc31-4b71-83e5-1292908139e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
        ...
        if [ ! -e "$S_FP" ]; then
            if [ -h "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='6e006a91-9abd-43c8-94e5-1292908139e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
                ...
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo -e "\e[31mdoes not exist\e[39m."
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='57f74414-394e-47aa-82e5-1292908139e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
            ...
            if [ ! -d "$S_FP" ]; then
                if [ "$SB_DISPLAY_VERIF_FAILURE_MSG" == "t" ]; then
                    ...
                    echo "GUID==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='b9a2d262-6cd8-4bce-a3d5-1292908139e7'"
                    echo ""
                fi
                #--------
                SB_VERIFICATION_FAILED="t"
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='b908d326-6c20-4896-a5d5-1292908139e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_add_bin_2_Z_PATH_and_optionally_share_man_2_MANPATH_t1(){
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_GUID_CANDIDATE" == "" ]; then
            ...
            echo "    S_GUID_CANDIDATE==\"\""
            echo ""
            echo "GUID=='6550881d-6edb-4c84-a5c5-1292908139e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        if [ "$S_FP_INSTALLATION_FOLDER" == "" ]; then
            ...
            echo "GUID=='47a4f3f4-fd62-4653-83c5-1292908139e7'"
            echo ""
            #--------
            SB_VERIFICATION_FAILED="t"
        fi
        ...
        if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "" ]; then
            ...
            if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "t" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" != "f" ]; then
                    ...
                    echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                    echo "GUID=='6b689236-89e2-4335-b3c5-1292908139e7'"
                    echo ""
                    #--------
                    SB_VERIFICATION_FAILED="t"
                fi
    ...
    if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
        func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
            "$S_FP_INSTALLATION_FOLDER" "0bb56446-4bba-414b-b3d6-1292908139e7"
        if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            #--------------
            func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                "$S_FP_INSTALLATION_FOLDER/bin" "f03c4722-0d93-45b6-93d6-1292908139e7"
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
            ...
            if [ "$SB_MAN_FOLDER_OR_NONBROKEN_SYMLINK_TO_IT_EXISTS" == "f" ]; then
                if [ "$SB_MAN_FOLDER_EXISTENCE_IS_MANDATORY" == "t" ]; then
                    ...
                    func_mmmv_verify_that_the_folder_exists_but_do_not_exit_t1 \
                        "$S_FP_INSTALLATION_FOLDER/share/man" "05ef1219-cd22-4c9d-a1c6-1292908139e7"
                fi
            else
    ...
    if [ "$SB_VERIFICATION_FAILED" != "t" ]; then
        if [ "$SB_VERIFICATION_FAILED" != "f" ]; then
            echo ""
            echo -e "\e[31mThe code of this Bash function is flawed.\e[39m"
            echo "GUID=='a8f67a5e-94ff-4277-82b5-1292908139e7'"
            echo ""
        fi
    fi
    #------------------------------
...
func_mmmv_assert_error_code_zero_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='9411695a-c8fe-4b19-a5a5-1292908139e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "GUID=='ac50ac35-e5ee-410e-a4a5-1292908139e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_error_code_zero_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='20333032-328f-4d57-b2a5-1292908139e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
    ...
    if [ "$S_ERR_CODE" != "0" ];then
        ...
        echo "Something went wrong. Error code: $S_ERR_CODE"
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='50c4e591-33f5-4973-8b95-1292908139e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        exit 1
...
func_mmmv_exc_verify_S_FP_ORIG_t1() {
    if [ "$S_FP_ORIG" == "" ]; then
        ...
        echo ""
        echo "Aborting script."
        echo "GUID=='5f123261-9821-4f44-b795-1292908139e7'"
        echo ""
    ...
    if [ ! -e "$S_FP_ORIG" ]; then
        ...
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='5778764c-af2e-4de1-9295-1292908139e7'"
        echo ""
        exit 1 # exit with an error
    fi
    #------------------------
    if [ ! -d "$S_FP_ORIG" ]; then
        ...
        echo "contains the script that prints this error message."
        echo -e "\e[31mAborting script. \e[39m"
        echo "GUID=='90974e82-9654-48e2-8685-1292908139e7'"
        echo ""
        exit 1 # exit with an error
    fi
} # func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_exc_verify_S_FP_ORIG_t2(){
    if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "t" ]; then
        if [ "$FUNC_MMMV_EXC_VERIFY_S_FP_ORIG_T2_S_FB_ORIG_ALREADY_VERIFIED" != "f" ]; then
            ...
            echo ""
            echo "has a domain of {\"f\", \"t\"}."
            echo "GUID=='2a19df8a-575a-431c-9385-1292908139e7'"
            echo ""
        else
            func_mmmv_exc_verify_S_FP_ORIG_t1
...
func_mmmv_cd_S_FP_ORIG_and_exit_t1(){
    ...
    cd "$S_FP_ORIG"
    func_mmmv_assert_error_code_zero_t2 "$?" \
        "7dce903c-f1f1-4cd9-95c6-1292908139e7"
    exit 0
} # func_mmmv_cd_S_FP_ORIG_and_exit_t1
...
func_mmmv_assert_nonempty_string_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='8f69b426-9138-46f6-8275-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='30e2845d-6347-4642-a475-1292908139e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_IN" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be something other than an empty string."
        echo "GUID=='c4212738-083b-4fb0-a565-1292908139e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_sbvar_domain_t_f_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='d130f915-4b6a-4594-a165-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ "$S_VARIABLE_NAME_IN_CALLING_CODE" == "" ]; then 
        ...
        echo "    S_VARIABLE_NAME_IN_CALLING_CODE==\"\""
        echo ""
        echo "GUID=='22db91ab-b70a-4de8-a155-1292908139e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$SB_VARIABLE_VALUE" != "t" ]; then 
        if [ "$SB_VARIABLE_VALUE" != "f" ]; then 
            ...
            echo ""
            echo "but it is expected to be either \"t\" or \"f\"."
            echo "GUID=='d4509d97-2322-42ac-9755-1292908139e7'"
            echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_assert_file_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='25b03063-4296-4b53-9455-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            ...
            echo "GUID=='58167e1a-b4fc-41f1-9245-1292908139e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='f5673639-df58-404e-8345-1292908139e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='9bee3919-aae8-463a-b335-1292908139e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='bf853c0a-f35b-416f-af35-1292908139e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='f396683f-4dde-45dc-8d25-1292908139e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='7b6a0054-332f-48cc-a225-1292908139e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_assert_folder_exists_t1() {
    ...
    if [ "$SB_LACK_OF_PARAMETERS" == "t" ]; then
        ...
        echo "GUID=='4b9febbe-a924-47f0-8325-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
        ...
        if [ "$SB_LACK_OF_PARAMETERS" != "f" ]; then
            echo -e "\e[31mThis code is flawed. \e[39m"
            echo "GUID=='30a10a1b-3781-49ea-8115-1292908139e7'"
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        fi
    ...
    if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "" ]; then
        ...
        if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "t" ]; then
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" != "f" ]; then
                ...
                echo "are: \"t\", \"f\", \"\"."
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
                echo "GUID=='14407c15-e079-4a32-be15-1292908139e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP" ]; then
        if [ -h "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='2a0fac56-355e-487d-8215-1292908139e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            echo -e "\e[31mdoes not exist\e[39m."
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='035a7188-6328-4c8f-8b05-1292908139e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
        ...
        if [ ! -d "$S_FP" ]; then
            ...
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo "GUID=='f2aab9b2-7dc9-447e-92f4-1292908139e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
            ...
            if [ "$SB_OPTIONAL_BAN_SYMLINKS" == "t" ]; then
                if [ -h "$S_FP" ]; then 
                    ...
                    echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                    echo "GUID=='5dcf2031-0c09-411f-b4f4-1292908139e7'"
                    echo ""
                    #--------
                    cd "$S_FP_ORIG"
                    exit 1 # exiting with an error
...
func_mmmv_exit_if_not_on_path_t2() { # S_COMMAND_NAME
    ...
    if [ "$S_LOCAL_VARIABLE" == "" ]; then
        ...
        echo "GUID=='4bf60552-55cb-4477-81f4-1292908139e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1;
    fi
...
func_mmmv_exc_exit_with_an_error_t1(){
    ...
    echo "GUID=='b1e12956-4232-4efb-84e4-1292908139e7'"
    echo ""
    cd "$S_FP_ORIG"
    exit 1 # exit with an error
} # func_mmmv_exc_exit_with_an_error_t1
...
func_mmmv_exc_exit_with_an_error_t2(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then 
        ...
        echo "Aborting script."
        echo "GUID=='7fd02a2d-d925-4fe6-81e4-1292908139e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    else
        ...
        echo "Aborting script."
        echo "GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo "GUID=='211728b9-9557-47b5-b3e4-1292908139e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_determine_Awk_command_t1() { # prefers the GNU Awk to the BSD Awk
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "5a428c1a-3148-44b9-a3c6-1292908139e7" 
    ...
    if [ "$SB_REINIT" == "t" ]; then
        ...
        local SB_THROW="f"
        local S_TMP_0="4a166a55-388a-404b-b1c6-1292908139e7"
        ...
        if [ "$SB_THROW" == "t" ]; then
            ...
            echo "The execution of this Bash script is aborted."
            echo "GUID=='efb4d822-0d06-48e1-94e4-1292908139e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1;
        fi
...
func_mmmv_exc_determine_sed_command_t1() { # prefers the GNU sed to the BSD sed
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "ea817e26-2e53-45cf-b3b6-1292908139e7" 
        SB_AVOID_REINIT="$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED"
    fi
    #----------------------------------------------------------------------
    local SB_REINIT="t"
    ...
    if [ "$SB_REINIT" == "t" ]; then
        ...
        local SB_THROW="f"
        local S_TMP_0="01f34c57-47cc-478c-a266-1292908139e7"
        local S_FP_0="/tmp/this/proB9bably/do00es/noTt/eXXXist/ㄫ/$S_TMP_0"
        #------------------------------------------------------------------
        ...
        if [ "$SB_THROW" != "f" ]; then
            ...
            echo "The execution of this Bash script is aborted."
            echo "GUID=='2804a843-1709-4c60-a8e4-1292908139e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1;
        fi
...
func_mmmv_exc_is_file_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then 
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='45e65ee1-b5d8-4e7b-89d4-1292908139e7'"
    ...
    if [ -d "$S_FP_CANDIDATE" ]; then 
        ...
        echo "GUID=='7c26e534-80cc-4ca8-82d4-1292908139e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_exc_is_folder_t1() {
    ...
    if [ ! -e "$S_FP_CANDIDATE" ]; then 
        ...
        echo ""
        echo -e "\e[31mdoes not exist\e[39m."
        echo "GUID=='d6248b54-37bd-4753-9cd4-1292908139e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
    if [ ! -d "$S_FP_CANDIDATE" ]; then 
        ...
        echo "GUID=='4a62f253-4af4-41a3-81c4-1292908139e7'"
        echo "Aborting without doing anything."
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_create_folder_t1(){
    ...
    if [ "$S_FP_FOLDER" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='7879f059-521d-4a3f-94c4-1292908139e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
    ...
    if [ -e "$S_FP_FOLDER" ]; then 
        if [ ! -d "$S_FP_FOLDER" ]; then 
            ...
            echo "Aborting script."
            echo "GUID=='dd22c539-33c5-40ed-81c4-1292908139e7'"
            echo ""
            cd "$S_FP_ORIG"
            exit 1 # exit with an error
        fi
        ...
        wait # just in case
        if [ "$S_TMP_0" != "0" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "06b9ab2b-9976-4d06-8466-1292908139e7" \
                "S_FP_FOLDER==$S_FP_FOLDER"
        fi
        if [ ! -e "$S_FP_FOLDER" ]; then 
            func_mmmv_exc_exit_with_an_error_t2 "34865321-8bd9-4263-b166-1292908139e7" \
...
func_mmmv_exc_assure_tmp_folder_existence_t1() {  # S_FP_TMP, S_GUID
    ...
    if [ "$S_GUID" == "" ]; then
        ...
        echo "GUID=='73b9cf42-f26c-4fcf-84c4-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    if [ ! -e "$S_FP_TMP" ]; then
        if [ -h "$S_FP_TMP" ]; then
            ...
            func_mmmv_assert_error_code_zero_t1 \
                "$?" "54657e63-c73d-4a91-8756-1292908139e7"
            func_mmmv_wait_and_sync_t1
            if [ -h "$S_FP_TMP" ]; then
                echo ""
                echo "The path "
                ...
                echo "GUID==\"$S_GUID\""
                echo "GUID=='ef9f732d-9607-407d-a2c4-1292908139e7'"
                echo ""
                #--------
                cd "$S_FP_ORIG"
                exit 1 # exiting with an error
        ...
        mkdir -p "$S_FP_TMP"
        func_mmmv_assert_error_code_zero_t1 \
            "$?" "2daa56d3-16b7-4f8d-9156-1292908139e7"
        func_mmmv_wait_and_sync_t1
        func_mmmv_assert_folder_exists_t1 "$S_FP_TMP" \
            '5d4c5f22-bdfb-413b-a2b4-1292908139e7'
    else
        if [ ! -d "$S_FP_TMP" ]; then
            echo ""
            if [ -h "$S_FP_TMP" ]; then
            ...
            echo "GUID==\"$S_GUID\""
            echo "GUID=='3c45d473-b87c-422b-a3b4-1292908139e7'"
            echo ""
            #--------
            cd "$S_FP_ORIG"
            exit 1 # exiting with an error
...
func_mmmv_ln_create_hardlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ "$SB_THROW_ON_INVALID_DATA" == "" ]; then
        ...
        if [ "$SB_THROW_ON_INVALID_DATA" != "t" ]; then
            if [ "$SB_THROW_ON_INVALID_DATA" != "f" ]; then
                ...
                echo ""
                echo "Aborting script."
                echo "GUID=='574a9a3d-129e-4739-91b4-1292908139e7'"
                echo ""
    ...
    if [ -e "$S_FP_LINK" ]; then 
        if [ -d "$S_FP_LINK" ]; then # folder or a symlink to a folder
            if [ ! -h "$S_FP_LINK" ]; then # not a symlink, therefore a folder
                ...
                echo "    $S_FP_TARGET"
                echo ""
                echo "GUID=='e06a7b81-4ee9-4e68-b4b4-1292908139e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            echo "GUID=='3307525b-64c8-4e77-b3b4-1292908139e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            if [ -h "$S_FP_LINK" ]; then # a broken symlink, therefore NOT missing
                ...
                echo "must not exist before the call to this function."
                echo "GUID=='28765b55-0010-49d9-b3a4-1292908139e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
        ...
        if [ -e "$S_FP_TARGET" ]; then
            if [ -d "$S_FP_TARGET" ]; then # a folder or a symlink to a folder
                if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore a folder
                    ...
                    echo "    $S_FP_LINK"
                    echo ""
                    echo "GUID=='5ab248b4-1635-4588-95a4-1292908139e7'"
                    echo ""
                    if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                        exit 1
                    fi
            ...
            if [ ! -h "$S_FP_TARGET" ]; then # not a symlink, therefore missing
                ...
                echo "    $S_FP_LINK"
                echo ""
                echo "GUID=='6487aa21-4f98-4c62-a5a4-1292908139e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
    ...
    #----------------------------------------------------------------------
    local S_TMP_0="not_set_yet GUID=='5bfc1350-019b-4720-a1a4-1292908139e7'"
    if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
        ...
        if [ "$S_TMP_0" != "0" ]; then
            ...
            echo "GUID=='25ec3822-20a0-447b-9594-1292908139e7'"
            echo ""
            if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                exit 1
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" == "f" ]; then
            ...
            if [ -h "$S_FP_TARGET" ]; then
                ...
                if [ ! -h "$S_FP_LINK" ]; then
                    ...
                    echo "GUID=='59a26b35-8679-4ba9-b294-1292908139e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -e "$S_FP_TARGET" ]; then # symlink to a folder or a file
                        if [ ! -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='477f3552-13f2-47f4-b194-1292908139e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ -d "$S_FP_TARGET" ]; then
                                if [ ! -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='7c67d541-cb8b-4863-b894-1292908139e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='76502d15-b02b-4160-9384-1292908139e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
                        ...
                        if [ -e "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='70c8d613-95e7-4d01-a384-1292908139e7'"
                            echo ""
                            S_TMP_0="t"
                        fi
                    fi
                ...
                if [ ! -e "$S_FP_TARGET" ]; then # just an extra test
                    echo ""
                    echo "Problem detection branch marker "
                    echo "GUID=='7cb1d61f-7465-428e-9384-1292908139e7'"
                    echo ""
                    S_TMP_0="t"
                else
                    if [ -d "$S_FP_TARGET" ]; then # just an extra test
                        echo ""
                        echo "Problem detection branch marker "
                        echo "GUID=='1ca46457-6cd1-4fee-8584-1292908139e7'"
                        echo ""
                        S_TMP_0="t"
                    else
                        if [ -h "$S_FP_LINK" ]; then
                            echo ""
                            echo "Problem detection branch marker "
                            echo "GUID=='1d3d6a3e-43e9-4737-9584-1292908139e7'"
                            echo ""
                            S_TMP_0="t"
                        else
                            if [ ! -e "$S_FP_LINK" ]; then
                                echo ""
                                echo "Problem detection branch marker "
                                echo "GUID=='9f135825-b1a3-4241-9374-1292908139e7'"
                                echo ""
                                S_TMP_0="t"
                            else
                                if [ -d "$S_FP_LINK" ]; then
                                    echo ""
                                    echo "Problem detection branch marker "
                                    echo "GUID=='3035b6e7-952f-4a65-b474-1292908139e7'"
                                    echo ""
                                    S_TMP_0="t"
                                fi
                            fi
            ...
            if [ "$S_TMP_0" == "t" ]; then
                ...
                echo "GUID=='a34bbe5c-67d5-41ce-8174-1292908139e7'"
                echo ""
                if [ "$SB_THROW_ON_INVALID_DATA" == "t" ]; then
                    exit 1
                fi
                ...
                if [ "$S_TMP_0" != "f" ]; then
                    ...
                    echo "Aborting script."
                    echo "GUID=='3af3472d-f718-4d1e-9574-1292908139e7'"
                    echo ""
                    exit 1
                fi
            fi
        ...
        if [ "$SB_DO_NOT_CREATE_THE_HARDLINK" != "t" ]; then
            ...
            echo -e "\e[31mThe implementation of this function is flawed. \e[39m"
            echo "Aborting script."
            echo "GUID=='dbac8059-89e9-4186-b564-1292908139e7'"
            echo ""
            exit 1
        fi
    fi
...
func_mmmv_assert_environment_variable_set_t1() { 
    ...
    if [ "$S_ENVIR_VALUE" == "" ]; then
        ...
        echo "GUID=='225d4ea1-2945-48f6-b564-1292908139e7'"
        echo ""
        exit 1 # exit with an error
    #else 
...
func_mmmv_iterate_over_array_02n_t1() {
    ...
    if [ "$S_ARRAY_VARIABLE_NAME" == "" ]; then
        ...
        echo "GUID=='d15515c8-5758-4b40-b164-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_ITERATION_FUNCTION_NAME" == "" ]; then
        ...
        echo "GUID=='1bd59b9e-cb87-4dcc-a464-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
...
func_mmmv_include_bashfile_if_possible_t1(){ # S_FP_BASHFILE S_GUID_CANDIDATE SB_THROW_ON_ERROR
    ...
    if [ "$SB_THROW_ON_ERROR" == "" ]; then
        ...
        if [ "$SB_THROW_ON_ERROR" != "t" ]; then
            if [ "$SB_THROW_ON_ERROR" != "f" ]; then
                ...
                echo "GUID=='eb983336-70c4-40e3-b164-1292908139e7'"
                echo ""
            fi
        fi
    fi
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='17fe9e17-6fdc-4fee-b254-1292908139e7'"
        echo ""
        if [ "$SB_THROW_ON_ERROR" == "t" ]; then
            exit 1
        fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='41f3c524-7f01-4def-a254-1292908139e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='a67434b5-dd7e-4eec-9454-1292908139e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1
            fi
    ...
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='afb90314-97cb-4d98-8554-1292908139e7'"
            echo ""
            if [ "$SB_THROW_ON_ERROR" == "t" ]; then
                exit 1 
            fi
...
func_mmmv_include_bashfile_if_possible_t2(){ # S_FP_BASHFILE S_GUID_CANDIDATE 
    ...
    if [ "$S_FP_BASHFILE" == "" ]; then
        ...
        echo "GUID=='3145bda3-7bed-45ab-b854-1292908139e7'"
        echo ""
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ ! -e "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='35045214-5d0b-46f3-b444-1292908139e7'"
            echo ""
        fi
    fi
    #-----------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        if [ -d "$S_FP_BASHFILE" ]; then
            ...
            echo "GUID=='51030132-1697-41ba-9444-1292908139e7'"
            echo ""
        fi
    fi
    #----------------------------------------------------------------------
    if [ "$SB_INCLUSION_POSSIBLE" == "t" ]; then
        ...
        if [ "$SB_INCLUSION_POSSIBLE" != "f" ]; then
            ...
            echo "GUID=='b5486419-fdcf-4ebc-b444-1292908139e7'"
            echo ""
        fi
    fi
...
func_mmmv_assert_exists_on_path_t1() {
    ...
    if [ "$S_TMP_1" == "" ]; then
        ...
        echo "GUID=='3d476850-2dfb-403a-b244-1292908139e7'"
        echo ""
        cd "$S_FP_ORIG"
        exit 1 # exit with an error
    fi
...
func_mmmv_ln_create_or_overwrite_symlink_t1() { # S_FP_TARGET  S_FP_LINK
    ...
    if [ -e "$S_FP_LINK" ]; then
        if [ ! -h "$S_FP_LINK" ]; then # not a symlink
            ...
            echo ""
            echo "PWD==`pwd`"
            echo "GUID=='a399211e-baa0-4da8-a344-1292908139e7'"
            echo ""
            exit 1 # exit with an error
        fi
    ...
    if [ "$SB_DELETE_OLD_LINK" == "t" ]; then
        rm -f $S_FP_LINK
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "3d806b3a-591c-44f4-a446-1292908139e7"
        func_mmmv_wait_and_sync_t1
        if [ -e "$S_FP_LINK" ]; then
            echo ""
            echo ""
            ...
            echo ""
            echo "PWD==`pwd`"
            echo "GUID=='5af20e52-2e74-4e79-9234-1292908139e7'"
            echo ""
            echo ""
            exit 1 # exit with an error
        fi
    ...
    $S_CMD_LN
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "982a05cb-597a-48da-8746-1292908139e7"
    func_mmmv_wait_and_sync_t1
    if [ ! -e "$S_FP_LINK" ]; then
        echo ""
        echo ""
        ...
        echo ""
        echo "PWD==`pwd`"
        echo "GUID=='d71eb72d-ef77-48f9-8134-1292908139e7'"
        echo ""
        echo ""
        exit 1 # exit with an error
    fi
...
func_mmmv_determine_operatingsystem_t1() {
    ...
    if [ "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "SB_AVOID_REINITIALISATION_IF_ALREADY_INITIALIZED" \
            "9195ca33-6df9-4a2f-9136-1292908139e7" 
    fi
...
func_mmmv_assert_Linux_or_BSD_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "GUID=='2ed4e3b9-98e1-4e48-8434-1292908139e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" == "" ]; then
        ...
        echo ""
        echo "GUID=='e6c2ce3f-eabc-445f-9134-1292908139e7'"
        echo ""
        # if [ "$S_FP_ORIG" != "" ]; then
        #     func_mmmv_exc_verify_S_FP_ORIG_t1
        #     cd "$S_FP_ORIG"
...
func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo ""
        echo "but it is expected to be a GUID."
        echo "GUID=='1ae7d6c3-2092-4c38-8124-1292908139e7'"
        echo ""
    fi
    #----------------------------------------------------------------------
            ...
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" != "f" ]; then
                ...
                if [ "$S_GUID_CANDIDATE" != "" ]; then
                    ...
                    func_mmmv_assert_sbvar_domain_t_f_but_do_not_exit_t1 \
                        "$SB_DISPLAY_NONERROR_FEEDBACK" \
                        "SB_DISPLAY_NONERROR_FEEDBACK" \
                        "3cb28e51-582e-40c4-a336-1292908139e7"
                fi
                SB_DISPLAY_NONERROR_FEEDBACK="f"
                #----------------------------------------
            fi
...
# RENESSAATOR_BLOCK_END
    ...
    if [ "$SB_DOMAIN_IS_REACHABLE" == "f" ]; then
        ...
        if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            echo "GUID=='2d67c971-a7ae-4292-b424-1292908139e7'"
            if [ "$S_GUID_CANDIDATE" != "" ]; then
                echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
            fi
        fi
...
func_mmmv_assert_internet_connection_exists_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='08f38116-72d5-4468-9424-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "3f25a253-b8e3-4dcd-8226-1292908139e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_STUDY_INTERNET_AVAILABILITY" == "t" ]; then
        ...
        # The 
        func_mmmv_verify_that_internet_connection_exists_but_do_not_exit_t1 \
            "fcca8d57-bcda-472a-9106-1292908139e7" \
            "$SB_DISPLAY_NONERROR_FEEDBACK"
        ...
        if [ "$SB_VERIFICATION_FAILED" == "t" ]; then
            ...
            if [ "$SB_VERIFICATION_FAILED" == "f" ]; then
                ...
                func_mmmv_assert_sbvar_domain_t_f_t1 \
                    ...
                    "SB_VERIFICATION_FAILED" \
                    "3ff79f64-870d-4c40-b806-1292908139e7"
            fi
        fi
        #----------------------------------------
    ...
    if [ "$SB_INTERNET_AVAILABLE" == "f" ]; then
        ...
        echo "GUID=='42d3912f-904f-4d18-9324-1292908139e7'"
        echo "S_GUID_CANDIDATE=='$S_GUID_CANDIDATE'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_mmmv_assert_GUI_applications_can_be_launched_t1(){
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='6d702d5c-3db0-4d2a-b424-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$SB_OK_TO_CACHE" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_OK_TO_CACHE" "SB_OK_TO_CACHE" \
            "4b013fb7-aedb-43a1-a3f5-1292908139e7"
    fi
    #----------------------------------------------------------------------
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        SB_DISPLAY_NONERROR_FEEDBACK="f"
    ...
    if [ "$SB_OK_TO_CACHE" == "t" ]; then
        if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_RESULT_CACHED" == "t" ]; then
            ...
            if [ "$SB_FUNC_MMMV_ASSERT_GUI_APPLICATIONS_CAN_BE_LAUNCHED_T1_GUI_IS_LAUNCHABLE" == "f" ]; then
                ...
                echo "Aborting script."
                echo "GUID=='57edbac4-cc36-4628-a314-1292908139e7'"
                echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
                echo ""
                #--------
                cd "$S_FP_ORIG"
    ...
    if [ "$SB_RUN_TEST" == "t" ]; then
        ...
        if [ "$S_TMP_0" != "124" ]; then
            ...
            echo "Aborting script."
            echo "GUID=='4dba3dc4-decc-4331-ab14-1292908139e7'"
            echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
            echo ""
            #--------
            cd "$S_FP_ORIG"
...
func_mmmv_exc_initialize_wine_C_drive_if_needed_t1(){
    ...
    if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "" ]; then
        ...
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            "$SB_DISPLAY_NONERROR_FEEDBACK" "SB_DISPLAY_NONERROR_FEEDBACK" \
            "5dc22742-dbe5-4ae4-83e5-1292908139e7" 
    fi
    #----------------------------------------------------------------------
    ...
    if [ ! -d "$S_FP_C" ]; then # for speed optimisation
        if [ ! -e "$S_FP_C" ]; then
            ...
            func_mmmv_assert_GUI_applications_can_be_launched_t1 \
                "e895265d-77d2-4ae0-a3c5-1292908139e7" \
            ...
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "18726ce5-3699-458a-a1c5-1292908139e7"
            func_mmmv_wait_and_sync_t1
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "11a0fb41-8672-4b0c-95c5-1292908139e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            if [ "$SB_DISPLAY_NONERROR_FEEDBACK" == "t" ]; then
            ...
            #--------------------------------------------------------------
            func_mmmv_assert_folder_exists_t1 "$S_FP_C" \
                "3cd442f4-2211-4223-86b5-1292908139e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            #--------------------------------------------------------------
        fi
    fi
...
func_mmmv_uncompress_tar_dot_foo_and_enter_t2(){
    ...
    if [ "$SB_COMPRESSED_FILE_WRAPS_A_SINGLE_TARFILE" != "" ]; then
        func_mmmv_assert_sbvar_domain_t_f_t1 \
            ...
            "SB_COMPRESSED_FILE_WRAPS_A_SINGLE_TARFILE" \
            "3551d047-f18d-4cc6-b1b5-1292908139e7"
    ...
    if [ "$SB_COMPRESSION_TYPE_RECOGNIZED" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='db561739-fc67-4857-b114-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1 # exiting with an error
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_PARENT_FOLDER" \
        "d29791a1-431b-441b-88a5-1292908139e7"
    cd $S_FP_PARENT_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "30748f33-d8b6-4740-94a5-1292908139e7"
    #----------------------------------------
    ...
    if [ "$SB_TAR_WRAPPED" == "t" ]; then
        ...
        if [ "$SB_COMPRESSIONLESS_TAR" == "f" ]; then
            func_mmmv_assert_file_exists_t1 "$S_FP_COMPRESSED" \
                "567b0155-3437-41b3-b395-1292908139e7"
            #--------
            if [ "$SB_MODE_ZIP" == "t" ]; then
                $S_CMD_NICE_PREFIX  zip --test $S_FP_COMPRESSED 
                func_mmmv_assert_error_code_zero_t1 "$?" \
                    "76d5a9eb-1ac7-4ac7-9295-1292908139e7"
            fi
            #--------
            $S_CMD_UNCOMPRESS $S_FP_COMPRESSED
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "308e4f14-eb7c-494a-9c95-1292908139e7"
            func_mmmv_wait_and_sync_t1
        fi
        #----------------------------------------
        func_mmmv_assert_file_exists_t1 "$S_FP_TAR" \
            "1ab532e5-46f8-492b-9285-1292908139e7"
        $S_CMD_NICE_PREFIX tar -xf $S_FP_TAR
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "82661644-0383-44a1-8285-1292908139e7"
        func_mmmv_wait_and_sync_t1
        #----------------------------------------
    else
        #----------------------------------------
        ...
        #----------------------------------------
        func_mmmv_assert_file_exists_t1 "$S_FP_COMPRESSED" \
            "5b5a1ea7-699b-4ae4-8285-1292908139e7"
        #--------
        if [ "$SB_MODE_ZIP" == "t" ]; then
            $S_CMD_NICE_PREFIX  zip --test $S_FP_COMPRESSED 
            func_mmmv_assert_error_code_zero_t1 "$?" \
                "3fd11e41-cce6-478c-b175-1292908139e7"
        fi
        #--------
        $S_CMD_UNCOMPRESS $S_FP_COMPRESSED
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "32384625-e1d6-4e10-8565-1292908139e7"
        func_mmmv_wait_and_sync_t1
        #----------------------------------------
    fi
    #----------------------------------------
    func_mmmv_assert_folder_exists_t1 "$S_FP_NEXT_PARENT_FOLDER" \
        "44c71a18-5992-411c-b265-1292908139e7"
...
func_mmmv_s_owner_of_a_file_or_folder_t1(){
    ...
    if [ ! -e "$S_FP_IN" ]; then
        if [ -h "$S_FP_IN" ]; then
            func_mmmv_exc_exit_with_an_error_t2 \
                "387ff7e2-5d2e-400c-9265-1292908139e7" \
                "The $S_FP_IN is a broken symlink."
        else
            func_mmmv_exc_exit_with_an_error_t2 \
                "1609a62a-1dfe-473c-9255-1292908139e7" \
...
func_mmmv_exc_block_device_ID_to_device_file_name_t1(){ # 
    ...
    if [ "$S_GUID_CANDIDATE" == "" ]; then
        ...
        echo "but it is expected to be a GUID."
        echo "Aborting script."
        echo "GUID=='1282d145-9b58-49d9-a414-1292908139e7'"
        echo ""
        #--------
        cd "$S_FP_ORIG"
        exit 1
    ...
    if [ "$S_BLOCK_DEVICE_ID_SUBPART_CANDIDATE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='dea51027-7729-416b-9204-1292908139e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_TMP_0" != "t" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='514e9901-c9d8-4e8a-a904-1292908139e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
    ...
    if [ "$S_FP_DEVICE_FILE" == "" ]; then
        ...
        echo "Aborting script."
        echo "GUID=='5cbad1fd-b3e5-4da9-b104-1292908139e7'"
        echo "S_GUID_CANDIDATE==\"$S_GUID_CANDIDATE\""
        echo ""
        #--------
        cd "$S_FP_ORIG"
...
func_delete_folder_of_build_folders(){
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_FOLDER_OF_BUILD_FOLDERS" \
        "63dc1d48-675a-4175-9555-1292908139e7" \
        "$SB_OPTIONAL_BAN_SYMLINKS"
    #------------------------------
    ...
    if [ -e "$S_FP_DELETABLE" ]; then
        ...
        echo ""
        echo "GUID=='47d25336-a19b-4ba3-9104-1292908139e7'"
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
        exit 1
        ...
        if [ -h "$S_FP_DELETABLE" ]; then # a broken symlink here
            ...
            echo "    S_FP_DELETABLE==\"$S_FP_DELETABLE\""
            echo ""
            echo "GUID=='3d4e4bc2-cdec-49cf-b904-1292908139e7'"
            echo ""
            #------------------------------
            cd "$S_FP_ORIG"
            exit 1
    ...
    if [ "$S_TMP_0" != "" ]; then # for safety
        ...
        func_mmmv_assert_error_code_zero_t1 "$?" \
            "38c9c1b1-db69-433b-9155-1292908139e7"
        func_mmmv_wait_and_sync_t1
        ...
        echo ""
        echo "GUID=='1bc1791e-e6da-4d65-a2f3-1292908139e7'"
        echo ""
        #------------------------------
        cd "$S_FP_ORIG"
        exit 1
...
func_first_use_message(){
    ...
    echo "GUID=='412b767a-eb9d-4106-a2f3-1292908139e7'"
    echo ""
    #------------------------------
    cd "$S_FP_ORIG"
    exit 1
...
func_display_help_message_and_exit_if_needed(){
    ...
    if [ "$S_ARGV_0" != "" ]; then
        if [ "`echo \"$S_ARGV_0\" | grep -E '^((([-][-]?)?[hH][eE][lL][pP])|([-]?[hH?]))$' `" == "" ]; then
            ...
            echo "GUID=='03d5a0ab-ad5b-4470-85f3-1292908139e7'"
...
func_initialize_configuration_by_initializing_global_variables(){
    ...
    S_FP_INSTALLATION_FOLDER="`cd $S_FP_DIR/../../; pwd `/v_0_4_8_15"
    S_FN_BASENAME="tor-0.4.8.15"
...
func_main(){
    ...
    func_mmmv_assert_folder_exists_t1 "$S_FP_DIR" \
        "1ae674c2-0cae-4d99-9245-1292908139e7"
    cd $S_FP_DIR
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "12b4172f-236a-42a0-9535-1292908139e7"
    #------------------------------
    func_mmmv_create_folder_t1 "$S_FP_BUILD_FOLDER"
    cd $S_FP_BUILD_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "522e6350-4cf2-43f9-9335-1292908139e7"
    #------------------------------
    func_mmmv_assert_file_exists_t1 "$S_FP_SRC" \
        "103dbf43-548b-4a85-9f35-1292908139e7"
    $S_CMD_PREFIX_NICE cp -f $S_FP_SRC $S_FP_BUILD_FOLDER/
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "31438b13-297f-45ed-a425-1292908139e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    ...
    cd $S_FP_PARENT_FOLDER
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "03105643-5932-4fb3-a125-1292908139e7"
    ...
    # # one of the tests that according to some
    # # rumors at the Wild-Wild-Web 
    # # is not even expected to pass, fails as "expected".
    # #--------
    # func_mmmv_assert_error_code_zero_t1 "$?" \
    #     "af057d26-92a0-4ff3-a205-1292908139e7"
    # func_mmmv_wait_and_sync_t1
    #------------------------------
    local S_TMP_0=""
    local SB_OPTIONAL_BAN_SYMLINKS="t"
    if [ -e "$S_FP_INSTALLATION_FOLDER" ]; then
        S_TMP_0="`echo \"$S_FP_INSTALLATION_FOLDER\" | grep -E '/v_0_4_8_15$' `"
        if [ "$S_TMP_0" != "" ]; then # for safety
            func_mmmv_assert_folder_exists_t1 "$S_FP_INSTALLATION_FOLDER" \
                "afbf5f38-8c3d-4c44-8305-1292908139e7" \
                "$SB_OPTIONAL_BAN_SYMLINKS"
            ...
            echo ""
            echo "GUID=='3aaa561e-f6bd-44b2-94f3-1292908139e7'"
            echo ""
            #------------------------------
            cd "$S_FP_ORIG"
            exit 1
    ...
    func_mmmv_assert_error_code_zero_t1 "$?" \
        "37600982-ab83-4dbd-b2f4-1292908139e7"
    func_mmmv_wait_and_sync_t1
    #------------------------------
    func_delete_folder_of_build_folders
    #------------------------------
    ...
    echo ""
    echo "Thank You for trying out this script. "
    echo "GUID=='5ba7e1e4-c19d-413d-a3f3-1292908139e7'"
    echo ""
    #------------------------------
} # func_main
func_main
...
exit 0
#==========================================================================
# S_VERSION_OF_THIS_FILE="8934cafc-1bf7-4e4d-b2f4-1292908139e7"
#==========================================================================
